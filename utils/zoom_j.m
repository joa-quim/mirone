...
% $Id: $
	...
	if strcmp(zoomCmd,'off')			% turn zoom off
		...
		if ~isempty(state)
			...
			%set(fig,'pointer',ptr)
			if (isappdata(fig,'ZOOMFigureState')),		rmappdata(fig,'ZOOMFigureState'),	end
	...
	switch zoomCmd
		case 'down'
			...
			if (m == 1)        % Zoom in
				...
				if (min(abs(ZOOM_Pt1-ZOOM_Pt2)) >= min(0.01*[diff(get_xlim(ax)) diff(get_ylim(ax))]))
...
%				state = uiclearmode_j(fig,'docontext','zoom_j',fig,'off');
		...
		case 'reset'
			axz = get(ax,'ZLabel');
			if isappdata(axz,'ZOOMAxesData'),	rmappdata(axz,'ZOOMAxesData'),	end
			return
		case 'xon'
	...
	if (~rbbox_mode)
		...
		a = [max(xmin,center(1)-dx) min(xmax,center(1)+dx) max(ymin,center(2)-dy) min(ymax,center(2)+dy)];
