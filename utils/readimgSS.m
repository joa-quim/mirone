function [lon, lat, z, ym] = readimgSS (file, west, east, south, north, scl)
% READIMGSS  Read a section of a Sandwell/Smith Mercator img file
%
...
% imgread.m 9545 2011-07-27 19:31:54Z pwessel
% P. Wessel, based on img2mergrd.c by Walter H.F. Smith
%
% J. Luis
%          Z is now an array of singles in case a conversion scale is applied
...
% $Id: readimgSS.m 4293 2014-01-19 18:10:28Z j $
	...
	equator = round(GMT_img_lat_to_ypix (0, nytop, radius));
	x_min = iinstart * dx;
	x_max = x_min + nx * dx;
	...
	if (scl ~= 1),		z = alloc_mex(ny, nx, 'single');	% Need a float because array will be scaled
	else				z = alloc_mex(ny, nx, 'int16');		% No scaling so use original data type
	end
	...
	for (jout = ny:-1:1)
		...
		k = (mod(row,2) == 1);				% Find odd values and adjust
		row(k) = row(k) - 1;
		z(jout,:) = row(ix);
	end

	if (scl ~= 1),		cvlib_mex('CvtScale', z, scl, 0),	end
...
% MERC_INV convert x,y to lon,lat
	...
	lat = 2 * atan(exp(y * pi/180)) * 180/pi - 90;
...
% measured downward in coordinate pixels.
	...
	f = nytop - radius * GMT_img_gud_inv(lat * pi/180);
...
% 0 on success.
	...
	nx360  = round (360 * 60 / mpixel);
	radius = nx360 / (2 * pi);
	nytop  = round (radius * GMT_img_gud_inv(maxlat * pi/180));
	nyrow  = nytop - round (radius * GMT_img_gud_inv(minlat * pi/180));
