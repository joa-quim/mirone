...
% $Id: gmtedit.m 10217 2018-01-24 21:33:46Z j $
	...
	uimenu(cmenuHand, 'Label', 'Overlay interpolation', 'Callback',@interp_on_grid);
	uimenu(cmenuHand, 'Label', 'Overlay another variable', 'Callback',@append_variable);

	item = uimenu(cmenuHand, 'Label', 'Show in XY grapher');
	uimenu(item, 'Label', 'All data', 'Callback',{@move_to_ecran, 'all'});
	uimenu(item, 'Label', 'This window data', 'Callback',{@move_to_ecran, 'window'});
...
%   x: absicssae vector
	...
	uicontrol('style','slider','units','pixels','Pos',Newpos, 'Callback',S,'min',x(1),'max',x(end)-width,'value',x(1));
