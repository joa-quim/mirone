...
% Pick (compute) the component that is select in GUI
	if (get(handles.radio_comp_z, 'Val'))
		if (comp > 13),		comp = 1:13;	end			% Do them all
		if (numel(comp) > 1)							% Add all selected components
			Z = h_in(handles.hfile, comp(1));
			for (k = 2:numel(comp))
				Z = Z + h_in(handles.hfile, comp(k));
			end
		else
	...
	elseif (get(handles.radio_comp_u, 'Val') || get(handles.radio_comp_ut, 'Val'))
		if (comp > 13),		comp = 1:13;	end			% Do them all
		if (numel(comp) > 1)							% Add all selected components
			Z = u_in(handles.ufile, comp(1), 'u');
			for (k = 2:numel(comp))
				Z = Z + u_in(handles.ufile, comp(k), 'u');
			end
		else
		...
		elseif (get(handles.radio_plotPhase, 'Val'))
			Z = atan2(-imag(Z), real(Z)) / pi*180;
			name = 'Phase ';
		else
			name = '';			% Just return Z
	...
	elseif (get(handles.radio_comp_v, 'Val') || get(handles.radio_comp_vt, 'Val'))
		if (comp > 13),		comp = 1:13;	end			% Do them all
		if (numel(comp) > 1)							% Add all selected components
			Z = u_in(handles.ufile, comp(1), 'v');
			for (k = 2:numel(comp))
				Z = Z + u_in(handles.ufile, comp(k), 'v');
			end
		else
...
% ----------------------------------------------------------------------------------------
function n = which_harmonics(handles)
% Make a vector with the selected harmonics
	n = zeros(13, 1);
	if (get(handles.check_M2, 'Val')),	n(1) = 1;	end
	if (get(handles.check_S2, 'Val')),	n(2) = 2;	end
	if (get(handles.check_N2, 'Val')),	n(3) = 3;	end
	if (get(handles.check_K2, 'Val')),	n(4) = 4;	end
	if (get(handles.check_K1, 'Val')),	n(5) = 5;	end
	if (get(handles.check_O1, 'Val')),	n(6) = 6;	end
	if (get(handles.check_P1, 'Val')),	n(7) = 7;	end
	if (get(handles.check_Q1, 'Val')),	n(8) = 8;	end
	if (get(handles.check_MF, 'Val')),	n(9) = 9;	end
	if (get(handles.check_MM, 'Val')),	n(10) = 10;	end
	if (get(handles.check_M4, 'Val')),	n(11) = 11;	end
	if (get(handles.check_MS4, 'Val')),	n(12) = 12;	end
	if (get(handles.check_MN4, 'Val')),	n(13) = 13;	end
	n(n == 0) = [];
...
% ...
	...
	for (k = 1:n_harmonic)
		...
		zi_i = grdtrack_m(imag(Z)', handles.head, [lon(:) lat(:)], '-Z');
		zi = zi_r + 1i*zi_i;
		if (~get(handles.radio_comp_z, 'Val') && (get(handles.radio_comp_u, 'Val') || get(handles.radio_comp_v, 'Val')))
			if (k == 1)
				[xy_lims,H] = grd_in(handles.gfile);		% reads in elevation for constituent COMP
				D = grdtrack_m(H', handles.head, [lon(:) lat(:)], '-Z');
			end
			zi = zi ./ D * 100;
		end
		amp(ic2,:) = abs(zi)';
