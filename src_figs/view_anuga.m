...
% M-File changed by desGUIDE 
	...
	handles.first = true;
	handles.one_or_zero = 0;
	handles.volumes = [];
	handles.hMirFig = [];
	handles.dms_xinc = 0;
	handles.dms_yinc = 0;
	handles.geog = 0;			% For the time beeing ANUGA no goga
...
function push_showSlice_Callback(hObject, eventdata, handles)
	...
	else											theVarName = 'ymomentum';
	end
	stage = nc_funs('varget', handles.fname, theVarName, [handles.sliceNumber 0], [1 handles.number_of_points]);
	handles.head(5:6) = [min(stage) max(stage)];
	
	% create a grid in x and y
	x = linspace(handles.head(1),handles.head(2),nx);
	y = linspace(handles.head(3),handles.head(4),ny);
	Z = mxgridtrimesh(handles.volumes, [handles.x(:) handles.y(:) stage(:)],x,y);
...
function view_anuga_LayoutFcn(h1,handles);
...
'Position',[520 548 350 280],...
'Resize','off',...
'Tag','figure1');

uicontrol('Parent',h1,...
'HorizontalAlignment','left',...
'Position',[299 180 40 15],...
'String','Slice nº',...
'Style','text',...
'Tag','text3');
...
'FontWeight','bold',...
'HorizontalAlignment','left',...
'Position',[10 260 100 17],...
'String','Input sww file',...
'Style','text',...
'Tag','text1');

uicontrol('Parent',h1,...
'Position',[10 182 70 15],...
'String','Real time',...
'Style','checkbox',...
