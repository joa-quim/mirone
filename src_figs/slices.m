...
% For compiling one need to include the aqua_suppfuns.m aquaPlugin.m files.
	...
	new_frame3D(hObject, handles.text_opt, [handles.frame1 handles.frame2])
	%------------- END Pro look (3D) -----------------------------------------------------
	...
	% --- Resize the figure into a minimal size. If needed, it will be expanded later ----
...
% Take care of moving uicontrols around an setting apropriate values for selected case.
	...
	if (val == 4),		set(handles.edit_periods, 'Str', '1:12')
	else				set(handles.edit_periods, 'Str', '8')
	end
...
uicontrol('Parent',h1, 'Position',[5 40 372 190], 'Style','frame', 'Tag','frame1');
...
uicontrol('Parent',h1,'Position',[0 332 382 3], 'Style','frame','Tag','frame2');
