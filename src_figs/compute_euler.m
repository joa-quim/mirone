...
% $Id: compute_euler.m 10051 2017-03-04 18:17:34Z j $
...
function check_hellinger_CB(hObject, handles)
	if (get(hObject,'Value'))
		...
		set([handles.check_show_stats handles.check_error_ellipse handles.check_in_equal_out ...
			handles.check_color_segmentation], 'Vis','on')
		for (k = 1:3)
		...
		set([handles.check_show_stats handles.check_error_ellipse handles.check_in_equal_out ...
			handles.check_color_segmentation], 'Vis','off')
	end
	...
	if (~get(handles.check_hellinger,'Val'))		% Our method
		...
		if (~isempty(conjug))		% Have Hellinger pair of isocs
			...
			[pLon, pLat, pAng, vol, stats, ellip_long, ellip_lat] = hellinger(handles.pLon_ini,handles.pLat_ini,handles.pAng_ini, ...
					handles.isoca1, handles.isoca2, handles.DP_tol, force_pole, false, props1, props2);
		else
			[area0, resid] = calca_pEuler(handles, false, false, []);
			[pLon, pLat, pAng, vol, stats, ellip_long, ellip_lat] = hellinger(handles.pLon_ini,handles.pLat_ini,handles.pAng_ini, ...
					handles.isoca1, handles.isoca2, handles.DP_tol, force_pole, get(handles.check_color_segmentation,'val'), resid);
		end
...
'Style','togglebutton',...
'Tooltip','Select the moving and fixed isochron with a double-click in each of the conjugate pair of lines',...
...
uicontrol('Parent',h1, 'Pos',[20 85 110 15],...
'Call',@compute_euler_uiCB,...
...
uicontrol('Parent',h1, 'Pos',[160 85 110 15],...
'String','Show stats',...
'Style','checkbox',...
...
uicontrol('Parent',h1, 'Pos',[300 85 120 15],...
...
'Visible', 'off', ...
'Tag','check_color_segmentation');

uicontrol('Parent',h1, 'Pos',[20 60 120 15],...
'String','Plot residues only',...
'Style','checkbox',...
'Tooltip','Plot the signed residues between isochron and rotated isochron. No pole is computed',...
'Tag','check_plotRes');
