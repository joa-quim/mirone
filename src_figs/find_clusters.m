...
% Choose default command line output for find_clusters_export
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes find_clusters_export wait for user response (see UIRESUME)
% uiwait(handles.figure1);

set(hObject,'Visible','on');
% NOTE: If you make uiwait active you have also to uncomment the next three lines
% handles = guidata(hObject);
% out = find_clusters_OutputFcn(hObject, [], handles);
% varargout{1} = out;

% --- Outputs from this function are returned to the command line.
%function varargout = find_clusters_OutputFcn(hObject, eventdata, handles)
...
%--------------------------------------------------------------------------
function checkbox_mainShock_Callback(hObject, eventdata, handles)
% Nothing to do here
...
if (get(handles.radiobutton_bigEvent,'Value'))
    mode = 'bigest';
elseif (get(handles.radiobutton_swarmCenter,'Value'))
    mode = 'center';
end
...
if (get(hObject,'Value'))
    ...
    set(handles.radiobutton_same,'Value',1)
end
...
if (get(hObject,'Value'))
    ...
    set(handles.radiobutton_separate,'Value',1)
end
...
if (get(handles.radiobutton_same,'Value'))    
...
% It makes use of the Mirone ImageCrop_Callback function, and also of the "register image"
% solution of draw_funs.

mirone('ImageCrop_Callback',gcbo,[],guidata(h_mirone_fig),h_polyg,'CropaWithCoords');
...
% Hints: contents = get(hObject,'String') returns popup_nSwarm contents as cell array
%        contents{get(hObject,'Value')} returns selected item from popup_nSwarm
...
uicontrol('Parent',h1,...
'Callback',{@find_clusters_uicallback,h1,'checkbox_mainShock_Callback'},...
'Position',[250 130 111 15],...
