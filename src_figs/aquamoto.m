...
handles = guihandles(hObject);
	...
	handles.ranges = cell(14,1);		% 14 = 3 (direct vars) + 11 (derived vars)
	handles.elevRange = [];
	...
	panel_names = {'anuga','t_grids','misc','shade','cinema'};
	...
	% The rest is donne in push_swwName_Callback() because only than we have all the necessary info
...
function edit_swwName_Callback(hObject, eventdata, handles)
	...
	if ( ~isempty(fname) ),    push_swwName_Callback([], [], handles, fname),	end
...
% This function does quite some work. It reads and extract relevant info from the netCDF file
	...
	if (any(ind))				% Not unlikely that some of the following are wrong. F... devided by (nearly) zero
		...
		handles.ranges{13} = D .* (1 + handles.ranges{4} .^ 2);			% D(1+V^2) - hazard_rvd_Range
...
% Find and plot the polygon which delimits the inundation zone
	...
	if (isempty(handles.hMirFig) || ~ishandle(handles.hMirFig))		% Do a lot of tricks 
		...
		push_showSlice_Callback(handles.push_showSlice, eventdata, handles)
...
function [theVar, U, V, indVar, indWater, qual] = get_derivedVar(handles)
	...
	switch qual(1:min(numel(qual),11))
		...
		case 'Max Depth'			% Maximum water depth (we need to compute it)
			...
			indVar = 9;
		case {'Velocity He' 'Energy (D +' 'Hazard-RVD ' 'Taylor-V (D'}	% V^2 / (2g) || D + V^2 / (2g) || D * (1 + V^2) || D*(1+V+V^2)
			x = nc_funs('varget', handles.fname, 'xmomentum', [handles.sliceNumber 0], [1 handles.number_of_points]);
			y = nc_funs('varget', handles.fname, 'ymomentum', [handles.sliceNumber 0], [1 handles.number_of_points]);
			if (~isa(x, 'double')),		x = double(x);		y = double(y);		end
...
function edit_sliceNumber_Callback(hObject, eventdata, handles)
	...
	set(handles.figure1,'pointer','watch')
	push_showSlice_Callback([], [], handles)		% Update image (also saves handles)
	set(handles.figure1,'pointer','arrow')
...
% -----------------------------------------------------------------------------------------
function radio_shade_Callback(hObject, eventdata, handles)
	if ( ~get(hObject,'Val'))
		set(hObject,'Val', 1)
		return
	end
...
% -----------------------------------------------------------------------------------------
function edit_multiLayerInc_Callback(hObject, eventdata, handles)
...
% -----------------------------------------------------------------------------------------
function radio_multiLayer_Callback(hObject, eventdata, handles)
...
% -----------------------------------------------------------------------------------------
function radio_timeGridsList_Callback(hObject, eventdata, handles)
...
function edit_landPhoto_Callback(hObject, eventdata, handles)
    fname = get(hObject,'String');
	if ( ~isempty(fname) )
    	push_landPhoto_Callback([], [], handles, fname)
	else					% Reset
		handles.geoPhoto = [];
		handles.useLandPhoto = false;
		guidata(handles.figure1,handles)
...
% -----------------------------------------------------------------------------------------
function push_landPhoto_Callback(hObject, eventdata, handles, opt)
...
% -----------------------------------------------------------------------------------------
function layerInc = edit_miscLayerInc_Callback(hObject, eventdata, handles)
% This function's code reuses the code of edit_multiLayerInc_Callback() because they
% are supposed to do exactly the same thing, which is - creating a vector map of layers to use
	...
	edit_multiLayerInc_Callback(hObject, eventdata, handles)
...
% 		HOWEVER, the Quan-Quach simply does not work as advertized (beter, it simply doesn't work)
...
'Callback',{@aquamoto_uicallback,h1,'tab_group_Callback'},...
'Enable','inactive',...
'Position',[132 399 50 23],...
'String','Misc',...
'ButtonDownFcn',{@aquamoto_uicallback,h1,'tab_group_ButtonDownFcn'},...
'Tag','tab_group',...
'UserData','misc');
...
h12 = uicontrol(...
'Parent',h1,...
'Callback',{@aquamoto_uicallback,h1,'push_batGrid_Callback'},...
'Position',[1001 455 21 21],...
...
'Parent',h1,...
'BackgroundColor',[1 1 1],...
'Callback',{@aquamoto_uicallback,h1,'edit_singleWater_Callback'},...
'HorizontalAlignment','left',...
'Position',[699 195 180 21],...
'Style','edit',...
...
h14 = uicontrol(...
'Parent',h1,...
'Callback',{@aquamoto_uicallback,h1,'push_singleWater_Callback'},...
...
'Parent',h1,...
'BackgroundColor',[1 1 1],...
'Callback',{@aquamoto_uicallback,h1,'edit_namesList_Callback'},...
'HorizontalAlignment','left',...
...
h16 = uicontrol(...
'Parent',h1,...
'Callback',{@aquamoto_uicallback,h1,'push_namesList_Callback'},...
...
'Parent',h1,...
'BackgroundColor',[1 1 1],...
'Callback',{@aquamoto_uicallback,h1,'listbox1_Callback'},...
'Position',[690 246 221 158],...
'Style','listbox',...
'Value',1,...
'Tag','listbox1',...
'UserData','t_grids');
...
h25 = uicontrol(...
'Parent',h1,...
'Callback',{@aquamoto_uicallback,h1,'radio_avi_Callback'},...
'FontName','Helvetica',...
'Position',[431 246 41 15],...
'String','AVI',...
'Style','radiobutton',...
...
h26 = uicontrol(...
'Parent',h1,...
'Callback',{@aquamoto_uicallback,h1,'radio_land_Callback'},...
'FontName','Helvetica',...
'FontSize',9,...
...
h27 = uicontrol(...
'Parent',h1,...
'Callback',{@aquamoto_uicallback,h1,'radio_water_Callback'},...
'FontName','Helvetica',...
'FontSize',9,...
...
h28 = uicontrol(...
'Parent',h1,...
'Callback',{@aquamoto_uicallback,h1,'radio_gif_Callback'},...
'FontName','Helvetica',...
'Position',[431 266 41 15],...
'String','GIF',...
'Style','radiobutton',...
...
h29 = uicontrol(...
'Parent',h1,...
'Callback',{@aquamoto_uicallback,h1,'checkbox_dither_Callback'},...
'FontName','Helvetica',...
'Position',[550 269 48 15],...
'String','Dither',...
'Style','checkbox',...
...
'Parent',h1,...
'BackgroundColor',[1 1 1],...
'Callback',{@aquamoto_uicallback,h1,'edit_fps_Callback'},...
...
h68 = uicontrol(...
'Parent',h1,...
'BackgroundColor',[0.990000009536743 0.990000009536743 0.990000009536743],...
'Callback',{@aquamoto_uicallback,h1,'slider_layer_Callback'},...
'Enable','inactive',...
...
'Parent',h1,...
'BackgroundColor',[1 1 1],...
'Callback',{@aquamoto_uicallback,h1,'edit_swwName_Callback'},...
'HorizontalAlignment','left',...
...
h70 = uicontrol(...
'Parent',h1,...
'Callback',{@aquamoto_uicallback,h1,'push_swwName_Callback'},...
...
'Parent',h1,...
'BackgroundColor',[1 1 1],...
'Callback',{@aquamoto_uicallback,h1,'edit_x_max_Callback'},...
'HorizontalAlignment','left',...
...
'Parent',h1,...
'BackgroundColor',[1 1 1],...
'Callback',{@aquamoto_uicallback,h1,'edit_y_max_Callback'},...
'HorizontalAlignment','left',...
...
'Parent',h1,...
'BackgroundColor',[1 1 1],...
'Callback',{@aquamoto_uicallback,h1,'edit_x_inc_Callback'},...
'HorizontalAlignment','left',...
...
'Parent',h1,...
'BackgroundColor',[1 1 1],...
'Callback',{@aquamoto_uicallback,h1,'edit_y_inc_Callback'},...
'HorizontalAlignment','left',...
...
'Parent',h1,...
'BackgroundColor',[1 1 1],...
'Callback',{@aquamoto_uicallback,h1,'edit_Ncols_Callback'},...
'HorizontalAlignment','left',...
...
'Parent',h1,...
'BackgroundColor',[1 1 1],...
'Callback',{@aquamoto_uicallback,h1,'edit_Nrows_Callback'},...
'HorizontalAlignment','left',...
...
'Callback',{@aquamoto_uicallback,h1,'push_Help_Callback'},...
'FontWeight','bold',...
...
'Parent',h1,...
'BackgroundColor',[1 1 1],...
'Callback',{@aquamoto_uicallback,h1,'edit_x_min_Callback'},...
'HorizontalAlignment','left',...
...
'Parent',h1,...
'BackgroundColor',[1 1 1],...
'Callback',{@aquamoto_uicallback,h1,'edit_y_min_Callback'},...
'HorizontalAlignment','left',...
...
h89 = uicontrol(...
'Parent',h1,...
'Callback',{@aquamoto_uicallback,h1,'push_showMesh_Callback'},...
'FontName','Helvetica',...
'FontSize',10,...
'FontWeight','bold',...
'Position',[146 72 100 23],...
'String','Show mesh',...
...
'Parent',h1,...
'BackgroundColor',[1 1 1],...
'Callback',{@aquamoto_uicallback,h1,'edit_sliceNumber_Callback'},...
'Enable','inactive',...
...
h92 = uicontrol(...
'Parent',h1,...
'Callback',{@aquamoto_uicallback,h1,'radio_stage_Callback'},...
'Position',[37 320 50 15],...
'String','Stage',...
'Style','radiobutton',...
...
h93 = uicontrol(...
'Parent',h1,...
'Callback',{@aquamoto_uicallback,h1,'radio_xmoment_Callback'},...
'Position',[136 320 85 15],...
'String','Xmomentum',...
'Style','radiobutton',...
...
h94 = uicontrol(...
'Parent',h1,...
'Callback',{@aquamoto_uicallback,h1,'radio_ymoment_Callback'},...
'Position',[263 320 80 15],...
'String','Ymomentum',...
'Style','radiobutton',...
...
'Parent',h1,...
'BackgroundColor',[1 1 1],...
'Callback',{@aquamoto_uicallback,h1,'popup_derivedVar_Callback'},...
'Enable','off',...
'Position',[131 279 190 22],...
...
h102 = uicontrol(...
'Parent',h1,...
'Callback',{@aquamoto_uicallback,h1,'push_showSlice_Callback'},...
'FontName','Helvetica',...
'FontSize',10,...
'FontWeight','bold',...
'Position',[260 72 100 23],...
'String','Show slice',...
...
'Parent',h1,...
'BackgroundColor',[1 1 1],...
'Callback',{@aquamoto_uicallback,h1,'edit_globalWaterMax_Callback'},...
...
'Parent',h1,...
'BackgroundColor',[1 1 1],...
'Callback',{@aquamoto_uicallback,h1,'edit_globalWaterMin_Callback'},...
...
h111 = uicontrol(...
'Parent',h1,...
'Callback',{@aquamoto_uicallback,h1,'push_checkGlobalMM_Callback'},...
'FontName','Helvetica',...
...
'Parent',h1,...
'BackgroundColor',[1 1 1],...
'Callback',{@aquamoto_uicallback,h1,'edit_maregs_Callback'},...
'HorizontalAlignment','left',...
...
h116 = uicontrol(...
'Parent',h1,...
'Callback',{@aquamoto_uicallback,h1,'push_maregs_Callback'},...
'Position',[1000 140 21 21],...
...
h118 = uicontrol(...
'Parent',h1,...
'Callback',{@aquamoto_uicallback,h1,'push_runIn_Callback'},...
'FontName','Helvetica',...
'FontSize',10,...
'FontWeight','bold',...
'Position',[30 72 100 23],...
'String','Plot Run In',...
...
h119 = uicontrol(...
'Parent',h1,...
'Callback',{@aquamoto_uicallback,h1,'radio_noShade_Callback'},...
'FontName','Helvetica',...
'FontSize',9,...
'Position',[370 503 82 17],...
'String','No shading',...
'Style','radiobutton',...
'Value',1,...
'Tag','radio_noShade',...
...
h120 = uicontrol(...
'Parent',h1,...
'Callback',{@aquamoto_uicallback,h1,'radio_shade_Callback'},...
'FontName','Helvetica',...
'FontSize',9,...
'Position',[371 473 70 17],...
'String','Shading',...
'Style','radiobutton',...
'Tag','radio_shade',...
'UserData','shade');
...
h121 = uicontrol(...
'Parent',h1,...
'Callback',{@aquamoto_uicallback,h1,'push_apply_Callback'},...
'Position',[370 420 81 23],...
'String','Apply',...
...
h124 = uicontrol(...
'Parent',h1,...
'Callback',{@aquamoto_uicallback,h1,'radio_multiLayer_Callback'},...
'FontName','Helvetica',...
'Position',[355 115 104 16],...
...
h125 = uicontrol(...
'Parent',h1,...
'Callback',{@aquamoto_uicallback,h1,'radio_timeGridsList_Callback'},...
'FontName','Helvetica',...
'Position',[490 115 104 16],...
...
h126 = uicontrol(...
'Parent',h1,...
'Callback',{@aquamoto_uicallback,h1,'push_interpolate_Callback'},...
'Enable','off',...
'Position',[690 110 110 23],...
'String','Interpolate',...
...
'Parent',h1,...
'BackgroundColor',[1 1 1],...
'Callback',{@aquamoto_uicallback,h1,'edit_miscLayerInc_Callback'},...
'Enable','off',...
'Position',[841 111 50 21],...
'String','1',...
'Style','edit',...
'TooltipString','Slice increment. ''1'' means all layers, ''2'', every other layer. The ML start:inc:end form is acepted as well',...
