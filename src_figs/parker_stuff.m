...
% $Id: parker_stuff.m 10217 2018-01-24 21:33:46Z j $
	...
	if (~isempty(varargin))
		...
		elseif (strcmp(varargin{1},'parker_inverse'))
			...
			handles.hCallingFig = varargin{2};
		elseif (strcmp(varargin{1},'redPole') || strcmp(varargin{1},'component'))
			no_igrf = true;
			...
			set(handles.edit_wlong,'Visible','off')
			set(handles.edit_wshort,'Visible','off')
			set(handles.edit_date,'Visible','off')
			set([handles.check_CenterDipole handles.check_fieldIsRTP],'Visible','off')
			set([handles.text_wshort handles.text_wlong handles.text_Date],'Visible','off')
			set([handles.check_showResidue handles.check_showDirect], 'Visible', 'off')
			...
			if (strcmp(varargin{1},'redPole'))
				set(hObject,'Name','Reduce to the Pole')
				handles.what_parker = 'redPole';
			else
				set(hObject,'Name','Component from total anomaly')
				handles.what_parker = 'component';
				set([handles.radio_X handles.radio_Y handles.radio_Z], 'Visible', 'on')
			end
			handles.hCallingFig = varargin{2};
...
function edit_date_CB(hObject, handles)
	if (strcmp(handles.what_parker,'redPole') || strcmp(handles.what_parker,'component'))
...
% -------------------------------------------------------------------------------------------------
function radio_X_CB(hObject, handles)
	if (~get(hObject,'Value')),		set(hObject,'Value',1),		return,		end
	set([handles.radio_Y handles.radio_Z],'Value',0)

% -------------------------------------------------------------------------------------------------
function radio_Y_CB(hObject, handles)
	if (~get(hObject,'Value')),		set(hObject,'Value',1),		return,		end
	set([handles.radio_X handles.radio_Z],'Value',0)

% -------------------------------------------------------------------------------------------------
function radio_Z_CB(hObject, handles)
	if (~get(hObject,'Value')),		set(hObject,'Value',1),		return,		end
	set([handles.radio_X handles.radio_Y],'Value',0)
...
% Source grid, Nrows & Ncols are common to all options. So test them first
	...
	switch (handles.what_parker)
		...
		case {'redPole' 'component'}
			...
			component = 0;
			fig_title = 'Reduce to the Pole anomaly';
			if (strcmp(handles.what_parker, 'component'))		% Compute one of the X,Y,Z comonents instead
				if     get(handles.radio_X, 'Val'),		component = 1;	fig_title = 'North component';
				elseif get(handles.radio_Y, 'Val'),		component = 2;	fig_title = 'East component';
				else,									component = 3;	fig_title = 'Vertical component';
				end
			end
			if (handles.orig_ncols < handles.ncols || handles.orig_nrows < handles.nrows)	% Padding was asked for
				if (get(handles.check_mirror,'Value'))   % Do mirror
					f = mboard(handles.Z_src,handles.orig_ncols,handles.orig_nrows);
					f = rtp3d(double(f),incl_fld,decl_fld,incl_mag,decl_mag, component);
				...
				f = rtp3d(double(handles.Z_src),incl_fld,decl_fld,incl_mag,decl_mag, 0);
			end
			z_min = min(f(:));    z_max = max(f(:));
			tmp.head = [handles.head_src(1:4) z_min z_max 0 handles.head_src(8:9)];
			tmp.X = handles.X;      tmp.Y = handles.Y;      tmp.name = fig_title;
			if (nargout == 0)				% Show result right away
...
set(h1, 'Position',[520 498 400 302],...
...
uicontrol('Parent',h1, 'Position',[50 232 311 21],...
'BackgroundColor',[1 1 1],...
'Callback',@parker_stuff_uiCB,...
'HorizontalAlignment','left',...
'Style','edit',...
'Tooltip','Enter bathymetry grid name (km +ve up)',...
'Tag','edit_BatGrid');

uicontrol('Parent',h1, 'Position',[361 231 23 23],...
'Callback',@parker_stuff_uiCB,...
'Tag','push_BatGrid');

uicontrol('Parent',h1, 'Position',[50 262 311 21],...
'BackgroundColor',[1 1 1],...
'Callback',@parker_stuff_uiCB,...
'HorizontalAlignment','left',...
'Style','edit',...
...
uicontrol('Parent',h1, 'Position',[50 210 140 15],...
'String','Geographic coords?',...
'Style','checkbox',...
'Tooltip','Are the grids in geographical coordinates?',...
'Tag','check_geog');

uicontrol('Parent',h1, 'Position',[225 210 140 15],...
'Callback',@parker_stuff_uiCB,...
...
uicontrol('Parent',h1, 'Position',[263 53 41 21],...
'BackgroundColor',[1 1 1],...
'Callback',@parker_stuff_uiCB,...
'Style','edit',...
...
uicontrol('Parent',h1, 'Position',[55 53 41 21],...
'BackgroundColor',[1 1 1],...
'Callback',@parker_stuff_uiCB,...
'String','0',...
'Style','edit',...
...
uicontrol('Parent',h1, 'Position',[162 53 41 21],...
'BackgroundColor',[1 1 1],...
'Callback',@parker_stuff_uiCB,...
'String','0.5',...
'Style','edit',...
...
uicontrol('Parent',h1, 'Position',[55 18 41 21],...
'BackgroundColor',[1 1 1],...
'Callback',@parker_stuff_uiCB,...
'Style','edit',...
...
uicontrol('Parent',h1, 'Position',[335 150 41 21],...
'BackgroundColor',[1 1 1],...
'Style','edit',...
'Tooltip','filter short wavelength cutoff (km)',...
'Tag','edit_wshort');

uicontrol('Parent',h1, 'Position',[335 110 41 21],...
'BackgroundColor',[1 1 1],...
'Style','edit',...
...
uicontrol('Parent',h1,'HorizontalAlignment','left','Position',[12 56 44 15],...
...
uicontrol('Parent',h1, 'Position',[19 139 60 15],...
'Callback',@parker_stuff_uiCB,...
'String','Mirror',...
'Style','checkbox',...
...
uicontrol('Parent',h1, 'Position',[126 102 51 78], 'BackgroundColor',[1 1 1],...
'Callback',@parker_stuff_uiCB,...
'Style','listbox','Value',1,'Tag','listbox_nny');

uicontrol('Parent',h1, 'Position',[232 102 51 78],...
'BackgroundColor',[1 1 1],...
'Callback',@parker_stuff_uiCB,...
'Style','listbox',...
'Value',1,...
...
uicontrol('Parent',h1, 'Position',[86 135 41 21],...
'BackgroundColor',[1 1 1],...
'Callback',@parker_stuff_uiCB,...
'Style','edit',...
...
uicontrol('Parent',h1, 'Position',[192 135 41 21],...
'BackgroundColor',[1 1 1],...
'Callback',@parker_stuff_uiCB,...
'Style','edit',...
...
uicontrol('Parent',h1, 'Position',[227 19 90 15],...
'Callback',@parker_stuff_uiCB,...
'String','Geocentric?',...
'Style','checkbox',...
...
uicontrol('Parent',h1, 'Position',[320 70 90 15],...
'String','Show res',...
'Style','checkbox',...
'Tooltip','Show also the residue grid (original anomaly - forward solution)',...
'Tag','check_showResidue');

uicontrol('Parent',h1, 'Position',[320 50 90 15],...
'String','Show direct',...
'Style','checkbox',...
'Tooltip','Show also the forward solution grid',...
'Tag','check_showDirect');

uicontrol('Parent',h1, 'Position',[326 11 65 21],...
'Callback',@parker_stuff_uiCB,...
'FontWeight','bold',...
'String','Compute',...
'Tag','push_compute');

uicontrol('Parent',h1, 'Position',[230 60 80 15],...
'Callback',@parker_stuff_uiCB,...
'String','North comp',...
'Style','radiobutton',...
'Value',0,...
'Tooltip','Compute X or North component',...
'Visible', 'off', ...
'Tag','radio_X');

uicontrol('Parent',h1, 'Position',[230 38 80 15],...
'Callback',@parker_stuff_uiCB,...
'String','East comp',...
'Style','radiobutton',...
'Value',0,...
'Tooltip','Compute Y or East component',...
'Visible', 'off', ...
'Tag','radio_Y');

uicontrol('Parent',h1, 'Position',[230 16 80 15],...
'Callback',@parker_stuff_uiCB,...
'String','Vert comp',...
'Style','radiobutton',...
'Value',1,...
'Visible', 'off', ...
'Tooltip','Compute Z or Vertical component',...
'Tag','radio_Z');
