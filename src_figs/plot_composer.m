...
% $Id: plot_composer.m 11427 2019-07-04 19:31:16Z j $
	...
	t = strtok(figname);		% Do some precautionary house keeping
	t = strrep(t, ' ', '');		t = strrep(t, ',', '');
	set(handles.edit_prefix,'String',t)
	% ---------------------------------------------------------------------------------------------------
...
% ...
	...
	function handles = create_images(handles, rect_x, rect_y, N, opt)
		...
		if (nargin == 4)
			handMir = guidata(handles.hAllFigs(N));
			if (~isempty(getappdata(handMir.figure1, 'IAmAMirone')))
				img = get(handMir.hImg, 'CData');
				n_rows = size(img,1);	n_cols = size(img,2);
				rows_tile = 128;		% Height of the thumb nail 
				fac = rows_tile / n_rows;
				cols_tile = round(n_cols * fac);
				img = cvlib_mex('resize', img, [rows_tile cols_tile], 'nearest');
...
% ...
	...
	pack.RJOK =  [' ' pb 'lim' pf ' ' pb 'proj' pf ' -O -K'];	% Changed mind, always spell the entire -R -J. No history
	...
	for (N = 1:numel(handles.hAllFigs))
		...
		elseif (strncmp(opt_J, '-JX', 3) && ~isempty(strfind(opt_J, '/0')) && isempty(handles.projection_str{N}))	% Linear, has freedom to deform
	...
	if (get(handles.radio_writeScript, 'Val'))
		if (~get(handles.check_runScript, 'Val'))
...
% Write the script initialization part (variables declaration)
	...
	if (~strcmp(handles.script_type, 'bat'))		% Write a bash script
		...
		script{l} = ['gmt gmtset PS_MEDIA=' paper_media]; l=l+1;
...
% Do the actual work of writing a pscoast command
	...
	if (~handMir.geog && handMir.is_projected)
		...
		if (handles.IamCompiled)
			ind = strfind(script{8}, '-J');
			script{8} = [script{8}(1:ind+1) 'x' escala];	% DANGEROUS. IT RELIES ON A FIXED INDEX
		else
...
nu = 1;
h(nu) = uicontrol('Parent',hFig, 'Position',[576 529 71 15],...
...
h(nu) = uicontrol('Parent',hFig, 'Position',[570 608 170 22],...
...
h(nu) = uicontrol('Parent',hFig, 'Position',[738 608 54 22],...
	'BackgroundColor',[1 1 1],...
	'Callback',@plot_composer_uiCB,...
	'String','',...
	'Style','popupmenu',...
...
h(nu) = uicontrol('Parent',hFig, 'Position',[576 553 161 22],...
	'BackgroundColor',[1 1 1],...
	'Callback',@plot_composer_uiCB,...
	'String','A4 595 842',...
	'Style','popupmenu',...
...
h(nu) = uicontrol('Parent',hFig, 'Position',[735 553 50 22],...
	'BackgroundColor',[1 1 1],...
	'Callback',@plot_composer_uiCB,...
	'String',{'cm'; 'in'; 'pt'},...
	'Style','popupmenu',...
...
h(nu) = uicontrol('Parent',hFig, 'Position',[658 529 75 15],...
	'Callback',@plot_composer_uiCB,...
	'String','Landscape',...
	'Style','radiobutton',...
	'UserData', figW-658, ...
...
h(nu) = uicontrol('Parent',hFig, 'Position',[655 207 61 21],...
	'BackgroundColor',[1 1 1],...
	'Callback',@plot_composer_uiCB,...
	'String','2.5',...
	'Style','edit',...
...
h(nu) = uicontrol('Parent',hFig, 'Position',[623 507 120 15],...
...
h(nu) = uicontrol('Parent',hFig, 'Position',[576 448 65 15],...
	'Callback',@plot_composer_uiCB,...
	'String','Auto run',...
	'Style','radiobutton',...
...
h(nu) = uicontrol('Parent',hFig, 'Position',[644 445 50 21],...
	'BackgroundColor',[1 1 1],...
...
h(nu) = uicontrol('Parent',hFig, 'Position',[576 417 105 15],...
	'Callback',@plot_composer_uiCB,...
...
h(nu) = uicontrol('Parent',hFig,'Position',[576 388 191 22],...
	'BackgroundColor',[1 1 1],...
	'Callback',@plot_composer_uiCB,...
	'String','Nikles',...
	'Style','popupmenu',...
...
h(nu) = uicontrol('Parent',hFig, 'Position',[765 390 21 21],...
	'Callback',@plot_composer_uiCB,...
	'FontWeight','bold',...
	'String','...',...
	'UserData', figW-765, ...
...
h(nu) = uicontrol('Parent',hFig, 'Position',[686 360 100 21],...
	'BackgroundColor',[1 1 1],...
	'String','',...
	'Style','edit',...
...
h(nu) = uicontrol('Parent',hFig, 'Position',[653 292 131 21],...
	'BackgroundColor',[1 1 1],...
	'Callback',@plot_composer_uiCB,...
	'String','',...
	'Style','edit',...
...
h(nu) = uicontrol('Parent',hFig, 'Position',[655 248 60 21],...
	'BackgroundColor',[1 1 1],...
	'Callback',@plot_composer_uiCB,...
	'String','',...
	'Style','edit',...
...
h(nu) = uicontrol('Parent',hFig, 'Position',[725 248 60 21],...
	'BackgroundColor',[1 1 1],...
	'Callback',@plot_composer_uiCB,...
	'String','',...
	'Style','edit',...
...
h(nu) = uicontrol('Parent',hFig, 'Position',[725 207 61 21],...
	'BackgroundColor',[1 1 1],...
	'Callback',@plot_composer_uiCB,...
	'String','2.5',...
	'Style','edit',...
...
h(nu) = uicontrol('Parent',hFig, 'Position',[600 252 51 15],...
...
h(nu) = uicontrol('Parent',hFig, 'Position',[671 271 35 16], 'String','Width', 'Style','text',...
	'UserData', figW-671, ...
	'Tag','text2');
	nu = nu + 1;

h(nu) = uicontrol('Parent',hFig, 'Position',[665 228 41 15],...
	'HorizontalAlignment','left',...
	'String','X origin',...
	'Style','text',...
	'UserData', figW-665, ...
...
h(nu) = uicontrol('Parent',hFig, 'Position',[735 228 41 15],...
	'HorizontalAlignment','left',...
	'String','Y origin',...
	'Style','text',...
	'UserData', figW-735, ...
...
h(nu) = uicontrol('Parent',hFig, 'Position',[625 363 58 15],...
	'HorizontalAlignment','left',...
	'String','Name prefix',...
	'Style','text',...
	'UserData', figW-625, ...
...
h(nu) = uicontrol('Parent',hFig, 'Position',[575 295 76 15],...
	'HorizontalAlignment','left',...
...
h(nu) = uicontrol('Parent',hFig, 'Position',[734 271 40 16], 'String','Height', 'Style','text',...
	'UserData', figW-734, ...
	'Tag','text9');
	nu = nu + 1;

h(nu) = uicontrol('Parent',hFig, 'Position',[600 210 51 15],...
	'HorizontalAlignment','left',...
	'String','Map origin',...
	'Style','text',...
	'UserData', figW-600, ...
...
h(nu) = uicontrol('Parent',hFig, 'Position',[636 582 70 15],...
	'FontSize',9,...
	'String','Paper size',...
	'Style','text',...
	'UserData', figW-636, ...
...
h(nu) = uicontrol('Parent',hFig, 'Position',[642 472 50 15],...
	'FontSize',9,...
	'String','Output',...
	'Style','text',...
	'UserData', figW-642, ...
...
h(nu) = uicontrol('Parent',hFig,'Position',[691 105 80 15],...
	'String','Time stamp',...
	'Style','checkbox',...
...
h(nu) = uicontrol('Parent',hFig, 'Position',[639 322 80 15],...
	'FontSize',9,...
	'String','Dimensions',...
	'Style','text',...
	'UserData', figW-639, ...
...
h(nu) = uicontrol('Parent',hFig, 'Position',[680 20 100 24],...
	'Callback',@plot_composer_uiCB,...
	'FontSize',9,...
	'FontWeight','bold',...
	'String','OK',...
