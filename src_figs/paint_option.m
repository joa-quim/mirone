...
switch str{val};
    ...
    case 'Clip'
        ...
        set(handles.checkbox_TransparentFg,'Enable','off')
        set(handles.checkbox_TransparentBg,'Enable','off')
...
% Hints: get(hObject,'String') returns contents of edit_OptionG_gray as text
%        str2double(get(hObject,'String')) returns contents of edit_OptionG_gray as a double
...
% Hint: get(hObject,'Value') returns toggle state of togglebutton_ClearFg
...
% Hint: get(hObject,'Value') returns toggle state of checkbox_TransparentBg
if ~isempty(get(handles.edit_OptionG_dpi,'String'))
    if get(hObject,'Value')
...
'singleton', 1, ...
'syscolorfig', 1, ...
'lastSavedFile', 'D:\m_gmt\paint_option.m', ...
'blocking', 0));
...
'Parent',h1,...
'BackgroundColor',[1 1 1],...
'Callback',{@paint_option_uicallback,h1,'edit_ShowCommand_Callback'},...
'HorizontalAlignment','left',...
