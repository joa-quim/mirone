...
% UIWAIT makes grid_calculator_export wait for user response (see UIRESUME)
% uiwait(handles.figure1);

set(hObject,'Visible','on');
% NOTE: If you make uiwait active you have also to uncomment the next three lines
% handles = guidata(hObject);
% out = grid_calculator_OutputFcn(hObject, [], handles);
% varargout{1} = out;

% ------------------------------------------------------------------------
function varargout = grid_calculator_OutputFcn(hObject, eventdata, handles)
...
try                             % Wrap it here to report any possible error
    ...
    elseif (prod(size(resp)) > 1)   % 'resp' is a array. Construct a fake grid
        [m,n] = size(resp);
        resp = single(resp);
        tmp.X = 1:n;        tmp.Y = 1:m;
