...
% $Id$
	...
	handles.path_continent = [home_dir fs 'continents' fs];
...
function radio_GEODVEL_CB(hObject, handles)
	...
	set([handles.radio_Nuvel1A handles.radio_Nuvel1A_NNR handles.radio_DEOS2K handles.radio_REVEL ...
		handles.radio_MORVEL handles.radio_PBird],'Val',0)
	set(handles.check_Abs2Rel,'Vis','off')
	...
	set(handles.popup_FixedPlate,'Value',1)
	set(handles.popup_MovingPlate,'Value',1)
	set(handles.popup_FixedPlate,'String',handles.GEODVEL_name)
	set(handles.popup_MovingPlate,'String',handles.GEODVEL_name)
...
function radio_DEOS2K_CB(hObject, handles)
	...
	% Fill the Moving plate popupmenu with the plate's names (we have to do this in every case)
...
% pole. This is because the pole list is relative to the Pacific plate. So, any other
...
%--------------------------------------------------------------------------------------------------
function push_poles_CB(hObject, handles)
% Select a pole from our list poles file
	fid = fopen([handles.path_continent 'lista_polos.dat'],'rt');
	c = fread(fid,'*char').';
	fclose(fid);
	s = strread(c,'%s','delimiter','\n');

	[s,v] = choosebox('Name','One Euler list',...		% v == 1  => Finite pole; v == 2 => Stage pole
	...
	if (s(4) <= 0),		s(4) = 1;	end			% So that instantaneous poles work as well
	set(handles.edit_PoleRate, 'String', num2str(abs(s(3)) / s(4)))
...
set(h1, 'Position',[520 559 282 241],...
...
uicontrol('Parent',h1,'Position',[100 140 101 94], 'Style','frame');
uicontrol('Parent',h1,'Position',[10 140 81 94],'Style','frame');

uicontrol('Parent',h1, 'Position',[10 93 121 22],...
'BackgroundColor',[1 1 1],...
'Callback',{@main_uiCB,h1,'popup_PickPlate_CB', 'fixed'},...
'Style','popupmenu',...
'Value',1,...
'Tag','popup_FixedPlate');

uicontrol('Parent',h1, 'Position',[150 93 121 22],...
'BackgroundColor',[1 1 1],...
'Callback',{@main_uiCB,h1,'popup_PickPlate_CB', 'mobile'},...
'Style','popupmenu',...
'Value',1,...
'Tag','popup_MovingPlate');

uicontrol('Parent',h1, 'Position',[10 115 52 15],...
'String','Fixed Plate',...
'Style','text');

uicontrol('Parent',h1, 'Position',[151 116 71 15],...
'HorizontalAlignment','left',...
'String','Moving Plate',...
'Style','text');

uicontrol('Parent',h1, 'Position',[14 207 71 15],...
'Callback',{@main_uiCB,h1,'radio_Nuvel1A_CB','Nuvel1A'},...
'String','Nuvel-1A',...
'Style','radiobutton',...
'Value',1,...
'Tag','radio_Nuvel1A');

uicontrol('Parent',h1, 'Position',[14 187 71 15],...
'Callback',{@main_uiCB,h1,'radio_Nuvel1A_CB','MORVEL'},...
'String','MORVEL',...
'Style','radiobutton',...
'Value',0,...
'Tag','radio_MORVEL');
...
'Callback',{@main_uiCB,h1,'radio_PBird_CB'},...
'String','P. Bird',...
'Style','radiobutton',...
'Tag','radio_PBird');

uicontrol('Parent',h1, 'Position',[14 147 87 15],...
'Callback',{@main_uiCB,h1,'radio_GEODVEL_CB'},...
'String','GEODVEL',...
'Style','radiobutton',...
'Tag','radio_GEODVEL');

uicontrol('Parent',h1, 'Position',[10 41 71 21],...
'BackgroundColor',[1 1 1],...
'Style','edit',...
'Tag','edit_PoleLon');

uicontrol('Parent',h1, 'Position',[10 63 72 15],...
'String','Pole Longitude',...
'Style','text');

uicontrol('Parent',h1, 'Position',[95 41 71 21],...
'BackgroundColor',[1 1 1],...
'Style','edit',...
'Tag','edit_PoleLat');

uicontrol('Parent',h1, 'Position',[93 63 72 15],...
'String','Pole Latitude',...
'Style','text');

uicontrol('Parent',h1, 'Position',[200 41 71 21],...
'BackgroundColor',[1 1 1],...
'Style','edit',...
'Tag','edit_PoleRate');

uicontrol('Parent',h1, 'Position',[200 63 72 15],...
'String','Rate (deg/Ma)',...
'Style','text');

uicontrol('Parent',h1, 'Position',[175 41 16 21],...
'Callback',{@main_uiCB,h1,'push_poles_CB'},...
'FontSize',8,...
'String','P',...
'ToolTip', 'Pick a pole from the internal poles list file',...
'Tag','push_poles');

uicontrol('Parent',h1, 'Position',[108 207 95 15],...
'Callback',{@main_uiCB,h1,'radio_Nuvel1A_NNR_CB'},...
'String','Nuvel-1A NNR',...
'Style','radiobutton',...
'Tag','radio_Nuvel1A_NNR');

uicontrol('Parent',h1, 'Position',[108 187 87 15],...
'Callback',{@main_uiCB,h1,'radio_DEOS2K_CB'},...
'String','DEOS2K',...
'Style','radiobutton',...
'Tag','radio_DEOS2K');

uicontrol('Parent',h1, 'Position',[108 167 87 15],...
'Callback',{@main_uiCB,h1,'radio_REVEL_CB'},...
'String','REVEL',...
'Style','radiobutton',...
'Tag','radio_REVEL');
...
uicontrol('Parent',h1, 'Position',[10 8 66 21],...
'Callback',{@main_uiCB,h1,'push_Help_CB'},...
'FontSize',9,...
'FontWeight','demi',...
...
uicontrol('Parent',h1, 'Position',[206 8 66 21],...
'Callback',{@main_uiCB,h1,'push_OK_CB'},...
'FontSize',9,...
'String','OK',...
'Tag','push_OK');
