...
% $Id: cartas_militares.m 10217 2018-01-24 21:33:46Z j $
...
% Check which tiles are selected and build a mosaic that embraces them all (even the inner non-selected ones)
	...
	figName = 'Mosaico LIDAR2011';
	if (~got_one)
		warndlg('Nickles de LIDAR grelhas no directório indicado. Bye Bye','Warning'),	return
	elseif (m * n == 1)		% When we have only one tile, use its own name
...
uicontrol('Parent',h1, 'Pos',[FigWidth-180-80 26 80 16],...
'FontName','Helvetica',...
'String','Resolução ',...
'Style','text',...
...
uicontrol('Parent',h1, 'Position',[FigWidth-180 27 40 19],...
'BackgroundColor',[1 1 1],...
'String',{'2' '10' '20' '50'},...
'Style','popupmenu',...
