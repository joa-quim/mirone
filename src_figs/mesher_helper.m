...
% $Id: mesher_helper.m 10217 2018-01-24 21:33:46Z j $
...
% This is used when calling this function via load_xyz() (load ASCII file)
	...
	if (~isempty(hLine))
		...
		uimenu(cmenuHand, 'Label', 'Rectangle limits (edit)', 'Callback', 'draw_funs([],''rectangle_limits'')');
		uimenu(cmenuHand, 'Label', 'New nested polygon', 'Callback', {@draw_new_nestPolyg, hLine});
		uimenu(cmenuHand, 'Label', 'Import new nested polygon', 'Callback', {@import_new_nestPolyg, hLine});
		uimenu(cmenuHand, 'Label', 'Delete', 'Callback', {@del_nestPolyg, hLine});
		uimenu(cmenuHand, 'Label', 'Save nested polygons', 'Callback', {@save_nesteds, hLine}, 'Sep','on');
...
% Draw the BoundingBox rectangle that will be the root polygon
	...
	uimenu(cmenuHand, 'Label', 'Rectangle limits (edit)', 'Callback', 'draw_funs([],''rectangle_limits'')');
	uimenu(cmenuHand, 'Label', 'New nested polygon', 'Callback', {@draw_new_nestPolyg, hPatch});
	uimenu(cmenuHand, 'Label', 'Import new nested polygon', 'Callback', {@import_new_nestPolyg, hPatch});
	uimenu(cmenuHand, 'Label', 'Delete', 'Callback', {@del_nestPolyg, hPatch});
	uimenu(cmenuHand, 'Label', 'Save nested polygons', 'Callback', {@save_nesteds, hPatch}, 'Sep','on');
...
% Browse for a data filename to be associated with the currently selected polygon
	...
	elseif (strcmp(drv, 'dat'))
		...
		if (isa(bin,'struct') || bin ~= 0)		% A binary file. For now I wont test any further
			...
			set([handles.radio_single handles.radio_double], 'Vis','on')			
...
% Plot the polygon x, y in the axes handMir.axes1 (the Mirone fig)
	...
	uimenu(cmenuHand, 'Label', 'Draw new nested polygon',   'Callback', {@draw_new_nestPolyg, hPat});
	uimenu(cmenuHand, 'Label', 'Import new nested polygon', 'Callback', {@import_new_nestPolyg, hPat});
	uimenu(cmenuHand, 'Label', 'Delete', 'Callback', {@del_nestPolyg, hPat});
	uimenu(cmenuHand, 'Label', 'Save nested polygons', 'Callback', {@save_nesteds, hPat}, 'Sep','on');
