...
% $Id$
	...
	if (nargin > 1 && ischar(varargin{2}) && length(varargin{2}) < 5)
		if (strcmpi(varargin{2},'igrf'))
			igrf_mode = 1;  xy_mode = 0;    xyz_mode = 0;
		elseif (strcmpi(varargin{2},'xy'))
			igrf_mode = 0;  xy_mode = 1;    xyz_mode = 0;
		elseif (strcmpi(varargin{2},'xyz'))
			igrf_mode = 0;  xyz_mode = 1;   xy_mode = 0;
		end
	...
	% -------- Make the listboxes
	ListBox = zeros(1,cols+1);		push_but = zeros(1,cols);
