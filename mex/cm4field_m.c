...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
	...
	int	argc = 0, nval = 0, nfval = 0, n_arg_no_char = 0;
	char	**argv;

	int	date_mmddyyyy = 0, date_year = 0, date_year_jul = 0;
	int	got_point = FALSE;
	double	*lon, *lat, *alt, *date, *out;
	...
	if (nrhs < 1 || nlhs == 10) {
		mexPrintf ("usage: out = cm4field(lon,lat,alt,year, '[-D<index>]', '[-f<10.7 flux>]', '[-F<thxyzdi[/1234567]>]', '[-S[cMin/Max][lMin/Max]]', '-G')\n");
		mexPrintf ("\tlon and lat are column or row vectors with the positions where to compute the field\n");
		mexPrintf ("\talt is the altitude in km positive up (it MUST be a scalar)\n");
		...
		mexPrintf ("\t-D<Dst_filename> full name of the 'Dst_all.wdc' file or a numeric Dst index.\n");
		mexPrintf ("\t-f<F107_filename> full name of the 'F107_mon.plt' file or a numeric F10.7 flux.\n");
		...
		mexPrintf ("\t  Append a number to indicate the requested field contribution(s)\n");
		mexPrintf ("\t	1 means Main field 1\n");
		mexPrintf ("\t	2 means Main field 2\n");
		mexPrintf ("\t	3 Primary Magnetospheric field\n");
		mexPrintf ("\t	4 Induced Magnetospheric field\n");
		mexPrintf ("\t	5 Primary ionospheric field\n");
		mexPrintf ("\t	6 Induced ionospheric field\n");
		mexPrintf ("\t	7 Toroidal field\n");
		mexPrintf ("\t  Use several numbers to add up the different contributions. For example\n");
		mexPrintf ("\t  -Ft/12 computes the total field due to Core and Lithospheric sources.\n");
		mexPrintf ("\t	The data is written out in the order specified in <dataflags>\n");
		mexPrintf ("\t	[Default is -Fthxyzdi/1]\n");
		mexPrintf ("\t-G Use geodetic instead of geocentric (default) coordinates.\n");
		mexPrintf ("\t-S[cMin/Max][lMin/Max] Minimum and maximum harmonic coefficients for core and\n");
		mexPrintf ("\t  lithospheric sources. Use one call for each source. Default is -Sc1/15 -Sl16/65.\n");
		return;
	}

	/* Read the location points */
	if (mxIsDouble(prhs[0]))
		lon = mxGetPr(prhs[0]);
	if (mxIsDouble(prhs[1]))
		lat = mxGetPr(prhs[1]);

	/* Count number of points where to compute the Field */
	n_pts = MAX(mxGetM(prhs[0]),mxGetN(prhs[0]));

	if (MAX(mxGetM(prhs[1]),mxGetN(prhs[1])) != n_pts)
		mexErrMsgTxt ("CM4 ERROR: lon and lat vector do not have the same number of points\n");
	...
	if (mxGetM(prhs[2]) > 1) {
		...
		if (MAX(mxGetM(prhs[2]),mxGetN(prhs[2])) != n_pts)
			mexErrMsgTxt ("CM4 ERROR: altitude vector is not of the same size as lon\n");
	}
	...
	if ((multiple_date = mxGetM(prhs[3])) > 1) {
		if (mxGetM(prhs[3]) != n_pts)
			mexErrMsgTxt ("CM4 ERROR: date vector is not of the same size as lon\n");
	}

	if (mxGetN(prhs[3]) > 1 && multiple_date)
		mexErrMsgTxt ("CM4 SORRY: multiple dates are only allowed with dates given in decimal years.\n");

	if (mxGetN(prhs[3]) == 1) {		/* We already know "year" */
		date_year = 1;
	}
	else if (mxGetN(prhs[3]) == 3) {
		ijour = (int)date[0];
		imon  = (int)date[1];
		ian   = (int)date[2];
		date_mmddyyyy = 1;
		...
		mexErrMsgTxt ("CM4 ERROR: date is in a unknown type.\n");
			...
			switch (argv[i][1]) {
				...
				case 'S':
					if (argv[i][2] == 'c') {
						...
						if (j != 2) 
							printf ("CM4 -S option ERROR, usage: -Sc<low/high>\n");
					}
					if (argv[i][2] == 'l') {
						j = sscanf (&argv[i][3], "%d/%d", &Ctrl->S.nlmf[1], &Ctrl->S.nhmf[1]);
						if (j != 2) 
							printf ("CM4 -S option ERROR, usage: -Sl<low/high>\n");
					}
					break;
			}
		}
	...
	if (got_point) {
		...
		Ctrl->DATA.alt = *alt;
		Ctrl->DATA.date = date;
		Ctrl->DATA.n_pts = n_pts;
		Ctrl->DATA.n_times = multiple_date;
	}
	/* ------------------------------------------------------------------ */
	...
	plhs[0] = mxCreateDoubleMatrix (Ctrl->DATA.n_pts, Ctrl->F.n_field_components, mxREAL);
	out = mxGetPr(plhs[0]);
	Ctrl->DATA.out_field = out;
	...
	if (Ctrl->D.index) {
		if (Ctrl->D.load) {
			...
			if ((fp = fopen(Ctrl->D.path, "r")) == NULL) {
				printf ("CM4: Could not open file %s\n", Ctrl->D.path);
				return 1;
			}
			jaft = 0;
			n = 0;
	...
	for (n = 0; n < Ctrl->DATA.n_pts; ++n) {
		...
		for (j = 0; j < Ctrl->F.n_field_components; j++) {	/* Loop over vector field components */
			...
			if (Ctrl->F.field_components[j] == 0) {
				t = sqrt(x*x + y*y + z*z); 
				Ctrl->DATA.out_field[n+j*Ctrl->DATA.n_pts] = t;
			}
			else if (Ctrl->F.field_components[j] == 1) {
				h = sqrt(x*x + y*y); 
				Ctrl->DATA.out_field[n+j*Ctrl->DATA.n_pts] = h;
			}
			else if (Ctrl->F.field_components[j] == 2)
				Ctrl->DATA.out_field[n+j*Ctrl->DATA.n_pts] = x; 
			else if (Ctrl->F.field_components[j] == 3)
				Ctrl->DATA.out_field[n+j*Ctrl->DATA.n_pts] = y; 
			else if (Ctrl->F.field_components[j] == 4)
				Ctrl->DATA.out_field[n+j*Ctrl->DATA.n_pts] = z; 
			else if (Ctrl->F.field_components[j] == 5)
				Ctrl->DATA.out_field[n+j*Ctrl->DATA.n_pts] = atan2(y,x) * R2D; 
			else if (Ctrl->F.field_components[j] == 6) {
				if (!h) h = sqrt(x*x + y*y);
				Ctrl->DATA.out_field[n+j*Ctrl->DATA.n_pts] = atan2(z,h) * R2D; 
			}
		}
	}
...
double intdst(int mjdl, int mjdh, int mjdy, int msec, double *dstx, int *cerr) {
	...
	if (jbot < mjdl || jtop > mjdh) {
		*cerr = 50;
		printf("SUBROUTINE INTDST -- ERROR CODE 50 -- T LIES OUTSIDE OF DST TABLE TIME SPAN -- ABORT\n");
		dst = -1e12;
	}
	else
...
double intf107(int iyrl, int imol, int iyrh, int imoh, int iyr, int imon, int idom, int *idim, int msec, double *f107x, int *cerr) {
	...
	if (ybot < iyrl || ytop > iyrh || ybot == iyrl && mbot < imol || ytop == iyrh && mtop > imoh) {
		printf("SUBROUTINE INTF107 -- ERROR CODE 50 -- T LIES OUTSIDE OF F10.7 TABLE TIME SPAN -- ABORT\n");
		f107 = -1.;
		*cerr = 50;
	}
	else
...
void getgmf(int nder, int ns, double *ep, double *tm, double *b, double *c, double *g, int *h, int *o, double *p) {
	...
	if (n > 0) {
		...
		printf("SUBROUTINE GETGMF -- ERROR CODE 56 -- T LIES OUTSIDE OF KNOT DOMAIN -- ABORT\n");
		return;
	    }
	    m = n + 1;
	    nk = k + 2;
...
void prebf_(int *rgen, int *ityp, int *etyp, int *dtyp, int *grad, int *nmni, int *nmxi, int *
	...
	if (MIN(i__1,*nmxe) < 0) {
		printf("SUBROUTINE BFIELD -- ERROR CODE 50 -- NMNI, NMXI, NMNE, OR NMXE < 0 -- ABORT\n");
		return;
	}
	i__1 = MIN(MIN(*mmni,*mmxi), *mmne);
	if (MIN(i__1,*mmxe) < 0) {
		printf("SUBROUTINE BFIELD -- ERROR CODE 51 -- MMNI, MMXI, MMNE, OR MMXE < 0 -- ABORT\n");
		return;
	}
	if (*mmni > *mmxi || *mmne > *mmxe) {
		printf("SUBROUTINE BFIELD -- ERROR CODE 52 -- EITHER MMNI > MMXI OR MMNE > MMXE -- ABORT\n");
		return;
	}
	if (*mmxi > *nmxi || *mmxe > *nmxe) {
		printf("SUBROUTINE BFIELD -- ERROR CODE 53 -- EITHER MMXI > NMXI OR MMXE > NMXE -- ABORT\n");
		return;
	}
	isvr = *ityp % 3;
	idst = *ityp / 3;
...
void fdlds_(int *rgen, int *grad, int *ctyp, double *clat, double *phi, double *h__, double *re, 
	...
	if (sinthe == 0.) {
	    if (*grad == 0)
		printf("SUBROUTINE BFIELD -- ERROR CODE 1 -- GEOGRAPHIC POLAR POSITION DETECTED, B-PHI INDETERMINABLE -- WARNING\n");
	    else
		printf("SUBROUTINE BFIELD -- ERROR CODE 2 -- GEOGRAPHIC POLAR POSITION DETECTED, B-PHI AND\n\t\t\t-- PHI-DERIVATIVE GRADIENT COMPONENTS INDETERMINABLE -- WARNING\n");
...
void tbspln_(double *t, int *n, int *k, double *bkpo, double *dtdb, int *cerr) {
	...
	printf ("TBSPLN -- ERROR CODE 54 -- T (%f) LIES OUTSIDE OF KNOT DOMAIN [%f; %f] -- ABORT\n", *t, bkpo[1], bkpo[*k + 2]);
	*cerr = 50;
    }
}
