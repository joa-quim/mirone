 ...
 * $Revision $  $Date: 2007/06/04 21:10:07 $
 */
...
void ind_to_sub(mwSize p, mwSize num_dims, mwSize *cumprod, mwSignedIndex *coords) {
    ...
    for (mwSize j_up = 0; j_up < num_dims; j_up++) {
        mwSize j = num_dims - 1 - j_up;
        coords[j] = static_cast<mwSignedIndex>(p / cumprod[j]);
        p = static_cast<mwSignedIndex>(p % cumprod[j]);
    }
}
...
Neighborhood_T allocate_neighborhood(mwSize num_neighbors, mwSize num_dims)
{
    ...
    result->array_coords = (mwSignedIndex *) mxCalloc(num_neighbors * num_dims,
...
static mwSize num_valid_scalars = sizeof(valid_scalars) / sizeof(double);
...
void nhCheckDomain(const mxArray *D)
{
    ...
    if (mxGetNumberOfElements(D) == 1)
    {
        ...
        for (mwSize k = 0; k < num_valid_scalars; k++)
        {
...
Neighborhood_T nhMakeDefaultConnectivityNeighborhood(mwSize num_dims)
{
    ...
    mwSize *unsigned_coords;

    num_neighbors = 1;
    size = (mwSize *)mxMalloc(num_dims * sizeof(*size));
    cumprod = (mwSize *)mxMalloc(num_dims * sizeof(*cumprod));
    unsigned_coords = (mwSize *)mxMalloc(num_dims * sizeof(*unsigned_coords));
    for (mwSize k = 0; k < num_dims; k++)
    {
        size[k] = 3;
    ...
    {
        mwSignedIndex *coords = result->array_coords + k*num_dims;
        ind_to_sub(k, num_dims, cumprod, unsigned_coords);
        for (mwSize q = 0; q < num_dims; q++)
        {
            coords[q] = static_cast<mwSignedIndex>(unsigned_coords[q]) - 1;
        }
    }
    
    mxFree(size);
    mxFree(cumprod);
    mxFree(unsigned_coords);
...
{
    mwSignedIndex *coords = nhood->array_coords;
    mwSize num_values = nhood->num_neighbors * nhood->num_dims;
    
    for (mwSize k = 0; k < num_values; k++)
    {
        coords[k] = -coords[k];
    }
}
 ...
 */
static
NeighborhoodWalker_T allocate_neighborhood_walker(mwSize num_neighbors, 
                                                  mwSize num_dims)
{
    NeighborhoodWalker_T walker;
    
    walker = (NeighborhoodWalker_T) mxMalloc(sizeof(*walker));
    walker->array_coords = (mwSignedIndex *) mxMalloc(num_neighbors*num_dims *
                                                sizeof(mwSignedIndex));
    walker->neighbor_offsets = (mwSignedIndex *) mxMalloc(num_neighbors * 
                                                    sizeof(mwSignedIndex));
    walker->image_size = (mwSize *) mxMalloc(num_dims * sizeof(mwSize));
    walker->cumprod = (mwSize *) mxMalloc((num_dims+1)* sizeof(mwSize));
    walker->center_coords = (mwSize *) mxMalloc(num_dims * sizeof(mwSize));
...
void process_walker_flags(NeighborhoodWalker_T walker, unsigned int flags)
{
    ...
    skip_center = (flags & NH_SKIP_CENTER) != 0;
    skip_leading = (flags & NH_SKIP_LEADING) != 0;
    skip_trailing = (flags & NH_SKIP_TRAILING) != 0;
    
    for (mwSize k = 0; k < walker->num_neighbors; k++)
    {
        mwSize num_dims = walker->num_dims;
        mwSignedIndex *pr = walker->array_coords + k * walker->num_dims;
...
NeighborhoodWalker_T nhMakeNeighborhoodWalker(Neighborhood_T nhood,
                                              ...
                                              mwSize num_input_dims,
                                              unsigned int flags)
{
    mwSize num_dims;
    NeighborhoodWalker_T walker;
    mwSize num_neighbors = nhood->num_neighbors;
    ...
    for (mwSize k = 0; k < num_neighbors; k++)
    {
        mwSignedIndex *pr_in = nhood->array_coords + k*nhood->num_dims;
        mwSignedIndex *pr_out = walker->array_coords + k*walker->num_dims;
        for (mwSize p = 0; p < nhood->num_dims; p++)
        {
            pr_out[p] = pr_in[p];
        }
         ...
         * are all 0.
         */
        for (mwSize p = nhood->num_dims; p < walker->num_dims; p++)
        {
            pr_out[p] = 0;
        }
    }

    for (mwSize k = 0; k < num_input_dims; k++)
    {
     ...
     * dimensions is 1.
     */
    for (mwSize k = num_input_dims; k < walker->num_dims; k++)
    {
        walker->image_size[k] = 1;
    }

    walker->cumprod[0] = 1;
    for (mwSize k = 1; k <= walker->num_dims; k++)
    {
    ...
    for (mwSize k = 0; k < num_neighbors; k++)
    {
        mwSignedIndex *pr_in = walker->array_coords + k*walker->num_dims;
...
void nhSetWalkerLocation(NeighborhoodWalker_T walker, mwSize p)
{
    ...
    ind_to_sub(p, walker->num_dims, walker->cumprod,
 ...
 */
static
bool is_inbounds_neighbor(mwSignedIndex *offset_coords, mwSize *array_coords, 
                          const mwSize *input_size, mwSize num_dims)
{
    bool result = true;
    ...
    for (mwSize k = 0; k < num_dims; k++)
    {
        mwSignedIndex coordinate = static_cast<mwSignedIndex>(array_coords[k]) + 
            offset_coords[k];
        if ((coordinate < 0) || (coordinate >= static_cast<mwSignedIndex>(input_size[k])))
        {
            result = false;
            break;
        }
