...
:simple
for %%G in (igrf_m scaleto8 tsun2 wave_travel_time mansinha_m telha_m range_change country_select 
...
link  /out:"gmtlist_m.%MEX_EXT%" %LINKFLAGS% %NETCDF_LIB% %GMT_LIB% /implib:templib.x gmtlist_m.obj 
IF "%1"=="GMT" GOTO END
REM ---------------------- END "GMTs" ----------------------------------------------
