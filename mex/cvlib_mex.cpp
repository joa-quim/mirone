/*--------------------------------------------------------------------
 *	$Id: cvlib_mex.cpp 4360 2014-03-28 01:55:17Z j $
 *
...
void Jmatmul(int n_out, mxArray *plhs[], int n_in, const mxArray *prhs[]);
...
void mexFunction(int n_out, mxArray *plhs[], int n_in, const mxArray *prhs[]) {
	...
	if (funName[0] == 'a' || funName[0] == 'A') {
		if (!strcmp(funName,"add") || !strcmp(funName,"addS") || !strcmp(funName,"abs") || 
			!strcmp(funName,"absDiffS") || !strcmp(funName,"absDiff"))
		...
		else if (!strcmp(funName,"avgstd") || !strcmp(funName,"A-mean") || 
		...
		if (!strcmp(funName,"eBox"))
			Jshapes(n_out, plhs, n_in, prhs, funName);

		else if (!strcmp(funName,"erode"))
			JerodeDilate(n_out, plhs, n_in, prhs, funName);

		else if (!strcmp(funName,"exp"))
			Jarithm(n_out, plhs, n_in, prhs, funName);
	}
	else if (funName[0] == 'f') {
		if (!strcmp(funName,"flip"))
			Jflip(n_out, plhs, n_in, prhs);

		else if (!strcmp(funName,"fillpoly"))
		...
		if (!strncmp(funName,"good", 4))
			JgoodFeatures(n_out, plhs, n_in, prhs);

		else if (!strcmp(funName,"gemm")) {
			Jmatmul(n_out, plhs, n_in, prhs);
		}
	}
	else if (funName[0] == 'h') {
		if (!strcmp(funName,"haar"))
		...
		else if (!strcmp(funName,"hypot"))
			Jarithm(n_out, plhs, n_in, prhs, funName);
	}
	else if (funName[0] == 'i') {
		if (!strcmp(funName,"inpaint"))
		...
		if (!strcmp(funName,"laplace"))
			Jedge(n_out, plhs, n_in, prhs, funName);

		else if (!strncmp(funName,"lin",3))
			Jshapes(n_out, plhs, n_in, prhs, funName);

		else if (!strcmp(funName,"log"))
			Jarithm(n_out, plhs, n_in, prhs, funName);
	}
	else if (funName[0] == 'm') {
		if (!strncmp(funName,"matchtemplate",5))
			JMatchTemplate(n_out, plhs, n_in, prhs);

		else if (!strcmp(funName,"mean")) 
			Jstat(n_out, plhs, n_in, prhs, funName);

		else if (!strncmp(funName,"morpho", 6))
			JmorphologyEx(n_out, plhs, n_in, prhs);

		else if(!strcmp(funName,"mul"))
			Jarithm(n_out, plhs, n_in, prhs, funName);
	}
	else if (funName[0] == 'n') {
		err++;
		...
		else if (!strncmp(funName,"rec",3))
			Jshapes(n_out, plhs, n_in, prhs, funName);
	}
	else if (funName[0] == 's') {
		if (!strcmp(funName,"sobel"))
			Jedge(n_out, plhs, n_in, prhs, funName);
		...
		else if (!strcmp(funName,"sub") || !strcmp(funName,"subS") || !strcmp(funName,"subRS"))
			Jarithm(n_out, plhs, n_in, prhs, funName);
...
void JapproxPoly(int n_out, mxArray *plhs[], int n_in, const mxArray *prhs[], const char *op) {
	...
	int j, nx, ny, np, geog = 0, do_DP = TRUE, return_pts = 1, is_closed, reclose = 0;
	...
	if (is_double && !geog) {
		...
		if (reclose) {		/* Add first point at the end so to (re)close the polygon */
			ptr_d[np-1] = ptr_d[0];		ptr_d[2*np-1] = ptr_d[np];
			if (nx == 3)
				ptr_d[3*np-1] = ptr_d[2*np];
		}
	}
	else if (is_single && !geog) {
		...
		if (reclose) {		/* Add first point at the end so to (re)close the polygon */
			ptr_s[np-1] = ptr_s[0];		ptr_s[2*np-1] = ptr_s[np];
			if (nx == 3)
				ptr_s[3*np-1] = ptr_s[2*np];
		}
	}
	else if (is_int && !geog) {
		...
		if (reclose) {
			ptr_i[np-1] = ptr_i[0];		ptr_i[2*np-1] = ptr_i[np];
		}
	}
	else {		/* The geographical coords case always returns the result in doubles */
...
/* --------------------------------------------------------------------------- */
void Jmatmul(int n_out, mxArray *plhs[], int n_in, const mxArray *prhs[]) {
	int nx1, nx2, ny1, ny2, nBands, nBands2, nBytes, img_depth, error = 0;
	int dims[2];
	...
	ny1 = mxGetM(prhs[1]);	nx1 = getNK(prhs[1],1);	nBands = getNK(prhs[1],2);
	src1 = cvCreateImageHeader( cvSize(nx1, ny1), img_depth, nBands );
	cvSetImageData( src1, (void *)mxGetData(prhs[1]), nx1 * nBytes * nBands );
	ny2 = mxGetM(prhs[2]);	nx2 = getNK(prhs[2],1);	nBands = getNK(prhs[2],2);
	src2 = cvCreateImageHeader( cvSize(nx2, ny2), img_depth, nBands );
	cvSetImageData( src2, (void *)mxGetData(prhs[2]), nx2 * nBytes * nBands );

	dims[0] = ny1;		dims[1] = nx2;
	plhs[0] = mxCreateNumericArray(mxGetNumberOfDimensions(prhs[1]),
  			dims, mxGetClassID(prhs[1]), mxREAL);
	dst = cvCreateImageHeader( cvSize(nx2, ny1), img_depth, nBands );

	cvSetImageData( dst, (void *)mxGetData(plhs[0]), nx2 * nBytes * nBands );

	cvGEMM(src1, src2, 1., NULL, 0., dst, 0);
