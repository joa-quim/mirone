...
void civmjd(int, int, int, int, int, double, int *, double*);
void setjd0(int, int, int);
void rge(double, double, double *, double *, double *, double, double, double);
void solid_ts(struct solid_time_series *TS);
void solid_grd(struct solid_time_series *TS, struct solid_grid *GRD);
void detide(double *, int, double, double *, double *, double *);
void mjdciv(int, double, int *, int *, int *, int *, int *, double *);
void geoxyz(double, double, double, double *, double *, double *);
void sunxyz(int, double, double *);
void moonxyz(int, double, double *);
void rot1(double, double, double, double, double *, double *, double *);
void rot3(double, double, double, double, double *, double *, double *);
void getghar(int, double, double * );
void st1isem(double *, double *, double *, double, double, double *);
void st1idiu(double *, double *, double *, double, double, double *);
void st1l1(double *, double *, double *, double, double, double *);
void sprod(double *, double *, double *, double *, double *);
void step2diu(double *, double, double, double *);
void step2lon(double *, double, double, double *);
...
int main(int argc, char **argv) {
	...
	if (TS.year < 1980)
		mexErrMsgTxt("Dates older than 1 Jan 1980 are not allowed in this program.\n");
...
void solid_grd(struct solid_time_series *TS, struct solid_grid *GRD) {
	...
	if (TS->year < 1980)
		mexPrintf("WARNING -- Dates older than 1 Jan 1980 underflow in gpsleap().\n");
	else if (TS->year > 2018)	/* No new leap seconds will before this date */
		mexPrintf("WARNING -- exceeded cut-off date in gpsleap(), accuracy loss is expected.\n");
...
void solid_ts(struct solid_time_series *TS) {
	...
	mjdciv(mjd, fmjd, &TS->year, &TS->month, &TS->day, &TS->hour, &TS->min, &TS->sec);	/* normalize civil time */
	setjd0(TS->year, TS->month, TS->day);
	tdel2 = 1.0 / 24 / 60;			/* 1 minute in GPS days */
...
/* ------------------------------------------------------------------------------- */
void detide(double *xsta, int mjd, double fmjd, double *xsun, double *xmon, double *dxtide) {
	...
	double tsecgps, xcorsta[3], inv_rsta;
	...
	tsecgps = fmjd * 86400.;			/* GPS time (sec of day) */
...
/* ----------------------------------------------------------------------- */
void step2lon(double *xsta, double fhr, double t, double *xcorsta) {
...
/* ------------------------------------------------------------------------------- */
void moonxyz(int mjd, double fmjd, double *rm) {
	...
	double cselat, selatd,  selond, fmjdtt, tsectt, tsecgps;

	/* ** use TT for lunar ephemerides */
	tsecgps = fmjd * 86400.;				/* GPS time (sec of day) */
	tsectt = gps2tt(tsecgps);				/* TT  time (sec ofday)  */
	fmjdtt = tsectt / 86400.;				/*  TT  time (fract. day)  */
...
/* ******************************************************************************* */
void getghar(int mjd, double fmjd, double *ghar) {
	...
	int i;
	double ghad, fmjdutc, tsecgps, tsecutc;
...
/* ******************************************************************************* */
void sunxyz(int mjd, double fmjd, double *rs) {
	...
	/* *** RAAN + arg.peri.  (deg.) */
	tsecgps = fmjd * 86400.;			/* GPS time (sec of */
	tsectt = gps2tt(tsecgps);			/* TT  time (sec of */
	fmjdtt = tsectt / 86400.;
	/* julian centuries since 1.5 january 2000 (J2000) */
	/*   (note: also low precision use of mjd --> tjd) */
	/*  TT  time (fract. */
...
/* ----------------------------------------------------------------------- */
void geoxyz(double lat, double lon, double eht, double *x, double *y, double *z) {
...
/* ----------------------------------------------------------------------- */
void rge(double gla, double glo, double *u, double *v, double *w, double x, double y, double z) {
...
/* ----------------------------------------------------------------------- */
void rot1(double theta, double x, double y, double z, double *u, double *v, double *w) {
...
/* ** time conversion **************************************************** */
/* *********************************************************************** */
void setjd0(int iyr, int imo, int idy) {
...
/* *********************************************************************** */
void civmjd(int iyr, int imo, int idy, int ihr, int imn, double sec, int *mjd, double *fmjd) {
...
void mjdciv(int mjd, double fmjd, int *iyr, int *imo, int *idy, int *ihr, int *imn, double *sec) {
...
L2:
	...
	if (mjd0t >= 57754)			/* 2017 JAN 1 = 57754 */
		...
		mexPrintf("FATAL ERROR -- fell thru tests in gpsleap()\n");
		return -1;
	}
