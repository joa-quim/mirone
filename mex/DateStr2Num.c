...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray*prhs[])
{
   ...
   if (nrhs == 1) {
      ...
      if (mxGetNumberOfElements(prhs[1]) != 1 || !mxIsNumeric(prhs[1])) {
         mexErrMsgIdAndTxt(ERR_ID   "BadFormatType", ERR_HEAD "2nd input [Format] must be a scalar number.");
      }
      ...
      switch (Format) {
         ...
         default:
           mexErrMsgIdAndTxt(ERR_ID   "BadFormat", ERR_HEAD "Format not supported.");
      }
   } else {
      mexErrMsgIdAndTxt(ERR_ID   "BadNInput", ERR_HEAD "1 or 2 inputs required.");
   }
   ...
   if (mxIsChar(C)) {         // Input is a string:
      ...
      for (iC = 0; iC < nC; iC++) {
         ...
         if ((aC = mxGetCell(C, iC)) == NULL) {  // Not initialized:
            mexErrMsgIdAndTxt(ERR_ID   "BadInputType", ERR_HEAD "Cell element is not a string.");
         }
      ...
      mexErrMsgIdAndTxt(ERR_ID   "BadInputType", ERR_HEAD "Input must be a string or a cell.");
...
double Str31Num(const mxArray *S)
{
  ...
  if (mxGetNumberOfElements(S) != 19) {
      mexErrMsgIdAndTxt(ERR_ID   "BadDateString",
                        ERR_HEAD "Bad size (%d) for [yyyy-mm-dd HH:MM:SS] format.", mxGetNumberOfElements(S));
  }
