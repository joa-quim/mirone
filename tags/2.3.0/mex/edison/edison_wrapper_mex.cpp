...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if ( nlhs >= 1 ) {	// first output - the feture space raw image
    ...
    if ( nlhs >= 2 ) {	// second output - labeled image
    ...
    delete [] labels;
    int arr_dims[2];
    if ( nlhs >= 3 ) {	// third output - the modes
        arr_dims[0] = N;
    ...
    if ( nlhs >= 4 ) {	// fourth output - region sizes (# of pixels)
        arr_dims[0] = 1;
        arr_dims[1] = RegionCount;
    ...
    if ( nlhs >= 5)	// fifth output - gradient map
        plhs[4] = mxgrad;
    else
