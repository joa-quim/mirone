...
void inv_cmap_2( mwSize colors, int32_T *colormap, uint32_T qm, uint32_T qe, 
...
void fsdither( uint8_T *r, uint8_T *g, uint8_T *b, 
    ...
    int32_T elevels, mask, bitshift0, bitshift1, bitshift2;
    int32_T *rq;
    int32_T *gq;
    int32_T *bq;
    int32_T *rq_next;
	...
	for (z = 0; z < m*n; z++) {
		...
		(((mwSize) zr) << (qm+qm)) + (((mwSize) zg) << qm) + ((mwSize) zb) ]); 
	}
	return;
    }
...
void ValidateInputs(int nrhs, const mxArray *prhs[], uint32_T *qm, uint32_T *qe) {
    mwSize p;
    mwSize q;
    mwSize i;
    real64_T *map;
    ...
    if ( (mxGetNumberOfDimensions(prhs[RGB]) == 3) && (size[2] != 3) ) {
    ...
    if (!mxIsDouble(prhs[QM]) || (mxGetNumberOfElements(prhs[QM]) != 1) || mxIsComplex(prhs[QM])) {
	mexErrMsgIdAndTxt("Images:ditherc:qmMustBeRealDoubleScalar",
                          "Qm must be a real double scalar.");
    }
    
    if (!mxIsDouble(prhs[QE]) || (mxGetNumberOfElements(prhs[QE]) != 1) || mxIsComplex(prhs[QE])) {
...
void mexFunction( int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    dithered_image = mxCalloc(mxGetNumberOfElements(result), sizeof(*dithered_image));
    inverse_colormap = mxCalloc( mlevels*mlevels*mlevels, sizeof(*inverse_colormap) );
