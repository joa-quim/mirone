 ...
 */
void check_nargin(double low, double high, int numInputs, const char *function_name) {
    mxArray *prhs[4];
    mxArray *plhs[1];
    int nlhs = 0;
    int nrhs = 4;
    ...
    mexCallMATLAB(nlhs, plhs, nrhs, prhs, "iptchecknargin");
                 ...
                 int           argument_position)
{
    ...
    mexCallMATLAB(nlhs, plhs, nrhs, prhs, "iptcheckinput");
...
mxArray *call_one_input_one_output_function(const mxArray *A, const char *function_name) {
    int nrhs = 1;
    int nlhs = 1;
    mxArray *prhs[1];
    mxArray *plhs[1];
...
bool is_inside(int p, int r_offset, int c_offset, int M, int N) {
    ...
    return ((new_col >= 0) && (new_col < N) && (new_row >= 0) && (new_row < M));
}
 ...
 * eight ints.
 */
extern void init_neighbors(int style, int M, int nhood_r[], int nhood_c[], int nhood[], int *num_neighbors) {
    int k;
