...
void ConvertFromDouble(double *pr, uint16_T *qr, mwSize numElements) {
    mwSize k;
    double val;
...
void ConvertFromSingle(float *pr, uint16_T *qr, mwSize numElements) {
    mwSize k;
    float val;
...
void ConvertFromUint8(uint8_T *pr, uint16_T *qr, mwSize numElements) {
    mwSize k;
    uint16_T val;
...
void ValidateInputs(int nrhs, const mxArray *prhs[]) {
    ...
    if (!mxIsDouble(prhs[0]) && !mxIsUint8(prhs[0]) && !mxIsSingle(prhs[0])) {
        mexErrMsgIdAndTxt("Images:grayto16:invalidType",
                          "%s","Input must be double, single, or uint8.");
    }
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    else if (mxIsUint8(prhs[0])) ConvertFromUint8((uint8_T *) mxGetData(prhs[0]), qr, 
