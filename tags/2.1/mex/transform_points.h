...
(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (X != NULL) {
        ...
        if (!mxIsDouble(X) || (mxGetNumberOfDimensions(X) > 2) || (mxGetN(X) != 2) || mxIsComplex(X))
            mexErrMsgIdAndTxt("Images:transform_points_h:inputXMustBeReal2D", "%s",ERR_X_BAD);
        ...
        for (k = 0; k < M*2; k++) {
            if (! mxIsFinite(X_ptr[k])) {
                mexErrMsgIdAndTxt("Images:transform_points_h:inputXMustBeFinite %s",ERR_X_NANINF);
            }
        }
    }
    else
    ...
    if ((T != NULL) && !mxIsEmpty(T)) {
        ...
        mexErrMsgIdAndTxt("Images:transform_points_h:inputTMustBeTFORMStruct %s",ERR_T_BAD);
    }
