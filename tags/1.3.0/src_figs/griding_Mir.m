...
griding_Mir_LayoutFcn(hObject,handles);
handles = guihandles(hObject);
...
else                    handles.home_dir = home_dir;    end
...
message = {'Input file has Header record(s). Number of header records can be changed'
           'on the "Nº of headers" box. Not used with binary data. This, like all GMT'
...
if (nargin == 4 & isstr(opt))   % OPT is a file name transmited by pushbutton_InputFile_Callback
    xx = opt;
...
if (strcmp(handles.command{1},'surface '))
    About_box(['surface_Last_modified_at_24_Oct_2004']);
elseif (strcmp(handles.command{1},'triangulate '))
    About_box(['triangulate_Last_modified_at_24_Oct_2004']);
else
    About_box(['nearneighbor_Last_modified_at_24_Oct_2004']);
end

% --- Creates and returns a handle to the GUI figure. 
function griding_Mir_LayoutFcn(h1,handles);
...
'singleton', 1, ...
'syscolorfig', 1, ...
'lastSavedFile', 'D:\m_gmt_so\griding_Mir.m', ...
'blocking', 0));
...
'Callback',{@griding_Mir_uicallback,h1,'pushbutton_Cancel_Callback'},...
'Position',[187 7 66 23],...
'String','Cancel',...
