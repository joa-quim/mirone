...
void MeanShift::MSVector(double *Mh_ptr, double *yk_ptr)
{
	...
	if(uniformKernel) {
		...
		for(i = 0; i < kp; i++) {
			t1 = h[i]*float(sqrt(offset[i]));
			for(j = 0; j < P[i]; j++) {
				range[2*(s+j)  ] = (float)(yk_ptr[s+j] - t1);
				range[2*(s+j)+1] = (float)(yk_ptr[s+j] + t1);
			}
			s += P[i];
		}
	}
	...
	for(i = 0; i < kp; i++)
    {
		...
		case Gaussian:
			...
			for(j = 0; j <= GAUSS_NUM_ELS; j++)
				w[i][j] = exp(-j*GAUSS_INCREMENT * 0.5);
				...
				if(diff < offset[j])
				{
					...
					for(j = 0; j < kp; j++)
					{
						if(kernel[j]) // not uniform kernel
						{
							...
							tw *= (((double)(x1)*increment[j] - u)*y0+(u - (double)(x0)*increment[j])*y1)/(double)(increment[j] * (x1 - x0));
