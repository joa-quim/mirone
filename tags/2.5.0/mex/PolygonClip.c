/* This is the gpc_mexfile.c of Sebastian Hölz  
 * Since he choosed to call the compiled library as PolygonClip
 * I renamed the mex gateway file to PolygonClip.c so that it
 * can be included in the logic of the Mirone makefiles. I also 
 * merged the gpc_mexfile.h contents here. Useless to have separated.
 *
 * I also corrected what I think was a bug. Returned polygons were not closed.
 * That is, last point != first point.
 *
 * Joaquim Luis	 30-Aug-2008
*/
/* ===========================================================================
	                               Includes
===========================================================================*/
...
void gpc_read_polygon_MATLAB (const mxArray *prhs, gpc_polygon *polygon);
void gpc_write_polygon_MATLAB(mxArray	    *plhs, gpc_polygon *polygon);
...
void gpc_write_polygon_MATLAB(mxArray *plhs, gpc_polygon *p) {
	...
	for (c=0; c < p->num_contours; c++) {	
		field_x = mxCreateDoubleMatrix(p->contour[c].num_vertices+1,1,mxREAL);
		field_y = mxCreateDoubleMatrix(p->contour[c].num_vertices+1,1,mxREAL);
		for (v = 0; v < p->contour[c].num_vertices; v++) {
			((double*)mxGetPr(field_x))[v] = p->contour[c].vertex[v].x;
			((double*)mxGetPr(field_y))[v] = p->contour[c].vertex[v].y;
		}
		/* There was a bug somewhere. We want a closed polygon. J. Luis */
		mxGetPr(field_x)[v] = p->contour[c].vertex[0].x;
		mxGetPr(field_y)[v] = p->contour[c].vertex[0].y;
