...
void mexFunction ( int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[] ) {
    ...
    switch ( nc_op->opcode) {
        ...
        default:
            ...
            sprintf ( error_message+strlen(error_message), "\tUnhandled opcode %d, %s, line %d, file %s\n", 
...
op *opname2opcode ( const char *opname, int nlhs, int nrhs ) {
    ...
    while (ops[j].opcode != NONE)    {
        ...
        if ( strcmp(p, ops[j].opname) != 0 ) /* This wasn't it.  Go on to the next.  */
            j++;
        else
            break;
    ...
    if (ops[j].opcode == NONE)    {
        sprintf ( error_message, "MEXNC ERROR:\n" );
        sprintf ( error_message+strlen(error_message), "\tNo such operation as \"%s\"\n", opname );
        mexErrMsgTxt(error_message);
    }
