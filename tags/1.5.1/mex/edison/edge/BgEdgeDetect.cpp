...
void BgEdgeDetect::GetPixels(int* nopix, int* pixx, int* pixy, double x1, double x2, double y1, double y2) {
   ...
   for (j=0; j<y_; j++) {
      k1 = j*x_;
      for (i=0; i<x_; i++) {
         if (permRank_[i+k1]<maxx && permRank_[i+k1]>minx &&
             permConf_[i+k1]<maxy && permConf_[i+k1]>miny) {
            pixx[npix] = i;
            pixy[npix++] = j;
         }
      }
...
void BgEdgeDetect::GetNmxPixels(int* nopix, int* pixx, int* pixy, double x1, double x2, double y1, double y2) {
   ...
   for (j=0; j<y_; j++) {
      k1 = j*x_;
      for (i=0; i<x_; i++) {
         if (permNmxRank_[i+k1]<maxx && permNmxRank_[i+k1]>minx &&
             permNmxConf_[i+k1]<maxy && permNmxConf_[i+k1]>miny) {
            pixx[npix] = i;
            pixy[npix++] = j;
         }
      }
...
void BgEdgeDetect::SubspaceEstim(float* im, float* grx, float* gry, float* cee)
{
   ...
   int i,j,l,c;
   double v1, D2R = 180.0/PI;
   double angleEdge;
   int WW2 = WW_*WW_, k1, k2;
   ...
   for (j=WL_; j<(y_-WL_); j++) {
      ...
      for (i=WL_; i<(x_-WL_); i++, itim++, itgx++, itgy++, itcee++) {
         if ((fabs(*itgx)+fabs(*itgy))>TOL_E) {
            angleEdge = (-atan2(*itgx, *itgy))*D2R;
            tae = lookTable_[(int) (angleEdge+180.49)];
            
            //A=A-mean(A)
            v1=0;
            for (l=0; l<WW_; l++) {
               k1 = l*WW_;	k2 = (l-WL_)*x_ - WL_;
               for (c=0; c<WW_; c++) {
                  v1 += ti[k1+c] = *(itim + k2 + c);
               }
            }
            v1 /= WW2;
            for (l=0; l<WW2; l++)
...
void BgEdgeDetect::GaussFilter(BgImage* cim, float* fim, double sigma, int width)
{
   ...
   for (j=0; j<y_; j++) {
      ...
      for (i=0; i<tail; i++) {
         ...
         for (k=-tail; k<=tail; k++) {
            ii = (k+i)>=0 ? k+i : 0;
            tim[k2] += im[k1 + ii]*filter[k+tail];
            ii = (x_-i-1+k) < x_ ? x_-i-1+k : x_-1;
            tim[k1 + x_-i-1] += im[k1+ii]*filter[k+tail];
         }
      }
   }
                                    ...
                                    float (BgEdgeDetect::*feval)(float, float, float, float))
{
   ...
   float* itnmxr;
   float* itnmxc;
   float alpha,r1,c1,r2,c2,lambda, lambda1;
   itr = rank;
   itc = conf;
   itgx = grx;
   itgy = gry;
      ...
      for (i=0; i<x_; i++, itr++, itc++, itgx++, itgy++, itnmxr++, itnmxc++) {
         if (*itr>0 && *itc>0) {
            ...
            if (alpha<=PI/4) {
               ...
               lambda1 = 1 - lambda;
               r1 = lambda1*(*(itr+1))+lambda*(*(itr+x_+1));
               c1 = lambda1*(*(itc+1))+lambda*(*(itc+x_+1));
               r2 = lambda1*(*(itr-1))+lambda*(*(itr-x_-1));
               c2 = lambda1*(*(itc-1))+lambda*(*(itc-x_-1));
               ...
               lambda1 = 1 - lambda;
               r1 = lambda1*(*(itr+x_))+lambda*(*(itr+x_+1));
               c1 = lambda1*(*(itc+x_))+lambda*(*(itc+x_+1));
               r2 = lambda1*(*(itr-x_))+lambda*(*(itr-x_-1));
               c2 = lambda1*(*(itc-x_))+lambda*(*(itc-x_-1));
               if ((this->*feval)(*itr, *itc, r1, c1)<0 && (this->*feval)(*itr, *itc, r2, c2)<=0) {
                  *itnmxr = *itr;
                  *itnmxc = *itc;
               }
