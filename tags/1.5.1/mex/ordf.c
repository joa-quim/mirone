                    ...
                    mwSize *Nd)
{
    ...
    if (nlhs > 1) {
        mexErrMsgIdAndTxt("Images:ordf:tooManyOutputs", "%s",
                          "Too many outputs.");
    }

    if (nrhs < 6) {
        mexErrMsgIdAndTxt("Images:ordf:toofewInputs", "%s",
                          "Too few inputs.");
    }
    if (nrhs > 7) {
        mexErrMsgIdAndTxt("Images:ordf:tooManyInputs", "%s",
                          "Too many inputs.");
    }
    ...
    *startCol = (mwSize) p[1] - 1;
    if ((((double) *startRow) != (p[0] - 1)) || (((double) *startCol) != (p[1] - 1))) {
    ...
    /* Get output size */
    if (!mxIsDouble(outsizeArray) || (mxGetNumberOfElements(outsizeArray) != 2)) {
    ...
    *Mb = (mwSize) p[0];
    *Nb = (mwSize) p[1];
    if ((((double) *Mb) != p[0]) || (((double) *Nb) != p[1])) {
    ...
    /* Get domain size */
    if (!mxIsDouble(domainsizeArray) || (mxGetNumberOfElements(domainsizeArray) != 2)) {
    ...
    *Md = (mwSize) p[0];
    *Nd = (mwSize) p[1];
    if ((((double) *Md) != p[0]) || (((double) *Nd) != p[1])) {
    ...
    /* Consistency checks */
    if ((*startRow >= mxGetM(*A)) || (*startCol >= mxGetN(*A)) ||
...
bool useHistMethodForINT8(mwSize numOffsets, mwSize Md, mwSize Nd) {
    ...
    */
    if( (numOffsets == Md*Nd) && (Md >= HIST8_REQ_ROWS) && (Nd >= HIST8_REQ_COLS) )
        return true;
    else
        return false;
} 
...
bool useHistMethodForINT16(mwSize numOffsets, mwSize Md, mwSize Nd) {
    ...
    */
    if( (numOffsets == Md*Nd) && (Md >= HIST16_REQ_ROWS) && (numOffsets >= HIST16_REQ_ELEMENTS) )
        return true;
    else
        return false;
} 
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    ValidateInputs(nlhs, nrhs, prhs, &A, &startRow, &startCol,
                   &order, &offsets, &numOffsets, &add, &Mb, &Nb, &Md, &Nd);
      ...
      case mxUINT8_CLASS:
        if( useHistMethodForINT8(numOffsets, Md, Nd) ) {
            ...
            ordfilt2_uint8((uint8_T *) mxGetData(A), (uint8_T *) mxGetData(B),
                           startRow, startCol, Mb, Nb, Ma, order, offsets, numOffsets);
        }
        break;
	...
	break;
      ...
      default:
        ...
        mexErrMsgIdAndTxt("Images:ordf:invalidInputClass", "%s", "Invalid input class.");
    }
