...
class ResampSep {
  ...
  private:   
    ...
    mwSize      GetCount( const mxArray* A );
    mwSize     *GetCumProd( const mxArray* A );

    Iterator   *NewIterator( const mwSize ndims, const mwSignedIndex* size );
    void        DestroyIterator(Iterator* i);
    inline void IncrementIterator(Iterator* i);
    inline bool DoneIterating(const Iterator* i);
    ...
    Kernel    **CreateKernelset( const mwSize nTrans, const mxArray* K );
    void        DestroyKernels( const mwSize nTrans, Kernel* kernelset[] );
    ...
    inline bool UseConvolution( const Convolver* c, const double* p );
    mwSignedIndex AdjustSubscript( const mwSignedIndex subscript, const mwSignedIndex tsize, 
                                 const PadMethod padMethod );
    void        InitConvolver( Convolver* c, const double* p );
    double      Convolve( const Convolver* c, double* v );


    mwSize     *CreateCPFill( const mwSize nOther, const mxArray* F );

    inline mwSignedIndex  Subs2Offset( const mwSize ndims, const mwSize* cumprod, 
    ...
    template<typename _T>
        void resample(_T *ptrBr, _T *ptrBi) {
            ...
            for( ResetIterator(f_oTransIt); !DoneIterating(f_oTransIt); IncrementIterator(f_oTransIt) ) {
                bool useConvolution = false;
                
                /* Cache transform portion of the output offset */
                mwSignedIndex oTransOffset = Subs2Offset(f_oCon->nTrans, f_oCon->cpTrans, f_oTransIt->subs);
                
                /* Cache output transform portion of the offset into M */
                mwSignedIndex mTransOffset = Subs2Offset(f_oCon->nTrans, f_cumprodM, f_oTransIt->subs);
                ...
                if( !mxIsNaN(ptrMr[mTransOffset]) ) {
                    ...
                    for( mwSize k = 0; k < f_iCon->nTrans; k++ ) {
                        mwSignedIndex mOffset = mTransOffset + k * (f_oCon->tlength);
                ...
                for( ResetIterator(f_otherIt); !DoneIterating(f_otherIt); IncrementIterator(f_otherIt) ) {
                    mwSignedIndex totalOutputOffset = oTransOffset
                        + Subs2Offset( f_oCon->nOther, f_oCon->cpOther, f_otherIt->subs );
                    double fReal = 0.0;
                    double fImag = 0.0;
                    double oReal = 0.0;
                    double oImag = 0.0;
                    ...
                    if( !bFempty ) {
                        mwSignedIndex fOffset = Subs2Offset(f_oCon->nOther, f_cpFill, f_otherIt->subs);
                        fReal = ptrFr[fOffset];
                        if( bComplexF ) fImag = ptrFi[fOffset];
                    }
                    
                    if( useConvolution ) {
                        mwSignedIndex iOtherOffset = Subs2Offset(f_iCon->nOther, f_iCon->cpOther, f_otherIt->subs);
                        
                        /* This is the convolution loop. */
                        for( mwSize j = 0; j < f_convolver->nPoints; j++ )  {
                            if( f_convolver->useFill[j] ) {
                                ...
                                mwSignedIndex totalInputOffset = 
                                    iOtherOffset + Subs2Offset(f_iCon->nTrans, f_iCon->cpTrans, f_convolver->subs[j]);
                    ...
                    if( bComplexA )
                        convert2Type((ptrBi + totalOutputOffset), oImag);
                }
            }
        }
