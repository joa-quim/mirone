...
% -------------------------------------------------------------------------
function [no,xo] = hist(y,x,min_max)
%HIST  Histogram.
...
if (nargin == 3)            % Min & Max of y was transmited in argument
    miny = min_max(1);
    maxy = min_max(2);
    got_min_max = 1;
...
for i=1:size(xx,2)
	...
	f = f';
	v = [xx(:,i) yy(:,i)];
	h=[h patch('faces', f, 'vertices', v, 'cdata', i*cc, 'FaceColor',facec,'EdgeColor',edgec)];
end
