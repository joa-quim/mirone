...
void sun_right_ascension_parallax_and_topocentric_dec(float latitude, float elevation, double xi, double h, double delta, double *delta_alpha, double *delta_prime) {
	...
	*delta_alpha = rad2deg(atan2(-x*sin(xi_rad)*sin(h_rad), cos(delta_rad) - x*sin(xi_rad)*cos(h_rad)));
	*delta_prime = rad2deg(atan2((sin(delta_rad) - y*sin(xi_rad))*cos(*delta_alpha), cos(delta_rad) - x*sin(xi_rad) *cos(h_rad)));
}
...
double sun_hour_angle_at_rise_set(float latitude, double delta_zero, double h0_prime) {
	...
	double argument = (sin(deg2rad(h0_prime)) - sin(latitude_rad)*sin(delta_zero_rad)) / (cos(latitude_rad)*cos(delta_zero_rad));
