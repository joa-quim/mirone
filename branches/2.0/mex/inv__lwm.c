...
#define ERR_CONV_HULL_VERTICES_BAD "ConvexHullVertices must be a full, real, double vector."
#define ERR_CONV_HULL_VERTICES_NANINF "ConvexHullVertices must not contain NaN or Inf."
#define ERR_CONV_HULL_VERTICES_INT "ConvexHullVertices must contain integers between 1 and V."
...
static void GetUV(double x, double y, LWMDataType *data, double *W, double *U, double *V) {
    mwSize i;
    double xy, x2, y2;
 ...
 */
static void LWMReverseMapping(LWMDataType *data, double x, double y, double *u, double *v) {
    double *W, *U, *V;
    ...
    if (denominator!=0.0) {
        ...
        *v = v_numerator/denominator;        
         ...
         */
        if (! data->warned) {
            mexWarnMsgIdAndTxt("Images:inv__lwm:"
...
static void DestroyLWMData(LWMDataType *data) {
    /*
...
void InitLWMData(const mxArray *T, LWMDataType *data) {
    ...
    mwSize k, nControlPoints = 0;
    const mwSize *dim_Tinv;
    ...
    if ((Tinv != NULL) && !mxIsEmpty(Tinv)) {
         ...
         * matrix.
         */
        if (!mxIsDouble(Tinv) || (mxGetNumberOfDimensions(Tinv) > 3) ||
            (dim_Tinv[0] != 6) || (dim_Tinv[1] != 2) || mxIsComplex(Tinv)) {
            mexErrMsgIdAndTxt("Images:inv__lwm:badLWMTdata", "%s",ERR_LWM_TDATA_BAD);
        }
        ...
        for (k = 0; k < mxGetNumberOfElements(Tinv); k++) {
            if (! mxIsFinite(pr_Tinv[k])) {
                mexErrMsgIdAndTxt("Images:inv__lwm:lwmTdataHasNansOrInfs", "%s",ERR_LWM_TDATA_NANINF);
            }
        }
    ...
    if ((ControlPoints != NULL) && !mxIsEmpty(ControlPoints)) {
         ...
         * matrix.
         */
        if (!mxIsDouble(ControlPoints) || (mxGetNumberOfDimensions(ControlPoints) > 2) ||
            (mxGetM(ControlPoints) != nControlPoints) ||
            (mxGetN(ControlPoints) != 2) || mxIsComplex(ControlPoints))
        {
            mexErrMsgIdAndTxt("Images:inv__lwm:badControlpoints", "%s",ERR_CONTROLPOINTS_BAD);
        }
        ...
        for (k = 0; k < nControlPoints*2; k++) {
            if (! mxIsFinite(pr_CP[k])) {
                mexErrMsgIdAndTxt("Images:inv__lwm:controlpointsHaveNansOrInfs",
    ...
    if ((RadiiOfInfluence != NULL) && !mxIsEmpty(RadiiOfInfluence)) {
         ...
         */
        if (!mxIsDouble(RadiiOfInfluence) || (mxGetNumberOfDimensions(RadiiOfInfluence) > 2) ||
            (mxGetNumberOfElements(RadiiOfInfluence) != nControlPoints) ||
            mxIsComplex(RadiiOfInfluence))
        {
            mexErrMsgIdAndTxt("Images:inv__lwm:badRadiiOfInfluence", "%s",ERR_RADIIOFINFLUENCE_BAD);
        }
        ...
        for (k = 0; k < nControlPoints; k++) {
            if (! mxIsFinite(pr_Radii[k])) {
                mexErrMsgIdAndTxt("Images:inv__lwm:radiiOfInfluenceHaveNansOrInfs",
                                  "%s",ERR_RADIIOFINFLUENCE_NANINF);
            }

            if (pr_Radii[k]<=0.0) {
                 mexErrMsgIdAndTxt("Images:inv__lwm:radiiOfInfluenceAreNegative",
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    if (nrhs < 2)
        mexErrMsgIdAndTxt("Images:inv__lwm:tooFewInputs", "%s",ERR_TOO_FEW_INPUTS);

    else if (nrhs == 2)
        TransformPoints(nlhs, plhs, nrhs, prhs);

    else
        mexErrMsgIdAndTxt("Images:inv__lwm:tooManyInputs", "%s",ERR_TOO_MANY_INPUTS);
}
