...
static mwSize weights2[2][2] = { {1, 4}, {2, 8} };

mwSize Nhood2Offset(mxLogical *pBWin, mwSize numRows, mwSize numCols, mwSize r, mwSize c) {
    ...
    for (rr = 0; rr <= maxR; rr++) {
        for (cc = 0; cc <= maxC; cc++)
            result += weights2[rr][cc] * (MATRIX_REF(pBWin, numRows, r+rr, c+cc) != 0);
    }

    return(result);
}

static mwSize weights3[3][3] = { {1, 8, 64}, {2, 16, 128}, {4, 32, 256} };

mwSize Nhood3Offset(mxLogical *pBWin, mwSize numRows, mwSize numCols, mwSize r, mwSize c) {

    mwSize minR, maxR, minC, maxC;
    mwSize rr, cc;
    mwSize result = 0;
...
void Compute2by2DoubleResult(mxArray *BWout, mxArray *BWin, mxArray *lut) {
    
    mwSize numRows, numCols;
    mwSize r, c;
    mxLogical *pBWin;
    double *plut;
    double *pBWout;
...
void Compute3by3DoubleResult(mxArray *BWout, mxArray *BWin, mxArray *lut) {
...
void Compute2by2Uint8Result(mxArray *BWout, mxArray *BWin, mxArray *lut) {
...
void Compute3by3Uint8Result(mxArray *BWout, mxArray *BWin, mxArray *lut) {
...
void Compute2by2LogicalResult(mxArray *BWout, mxArray *BWin, mxArray *lut) {
...
bool is3x3LogicalLutZeroInZeroOut(double *lut) {
    bool result = true;
    int lut_length = 512;   /* always 512 for 3x3 lookup table */
    uint16_T bit_mask = 16; /* on-bit corresponding to neighborhood center */
    uint16_T k;
    
    
    for (k = 0; k < lut_length; k++) {
        bool neighborhood_has_zero_center = (k & bit_mask) == 0;
        if ((neighborhood_has_zero_center) && (lut[k] != 0.0)) {
            result = false;
            break;
        }
    }
...
bool is3x3LogicalLutOneInOneOut(double *lut) {
    bool result = true;
    int lut_length = 512;   /* always 512 for 3x3 lookup table */
    uint16_T bit_mask = 16; /* on-bit corresponding to neighborhood center */
    uint16_T k;
    ...
    for (k = 0; k < lut_length; k++) {
        ...
        if ((neighborhood_has_one_center) && (lut[k] == 0.0)) {
            result = false;
            break;
        }
    }
...
void Compute3by3LogicalResult(mxArray *BWout, mxArray *BWin, mxArray *lut) {
    
    mwSize numRows, numCols;
    mwSize r, c;
    mxLogical *pBWin;
    mxLogical *pInputPixel;
    double *plut;
    mxLogical *pBWout;
    ...
    if (is3x3LogicalLutZeroInZeroOut(plut)) {
        pInputPixel = pBWin;
        for (c = 0; c < numCols; c++) {
            for (r = 0; r < numRows; r++) {
                if (*pInputPixel++ != 0)
                    *pBWout = (mxLogical) *(plut + Nhood3Offset(pBWin, numRows, numCols, r, c));
                pBWout++;
            }
        }
    }
    else if (is3x3LogicalLutOneInOneOut(plut)) {
        pInputPixel = pBWin;
        for (c = 0; c < numCols; c++) {
            for (r = 0; r < numRows; r++) {
                if (*pInputPixel++ == 0)
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    ValidateInputs(nrhs, prhs, &BW, &freeBW, &lut, &freelut, 
                   &outputClass, &nhoodSize);

    if (outputClass == mxLOGICAL_CLASS) {
	BW2 = mxCreateLogicalArray(mxGetNumberOfDimensions(BW), mxGetDimensions(BW));
    } else {
