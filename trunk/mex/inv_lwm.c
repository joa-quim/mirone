...
void InitLWMData(const mxArray *T, LWMDataType *data) {
    ...
    if ((Tinv != NULL) && !mxIsEmpty(Tinv)) {
        ...
        if (!mxIsDouble(Tinv) || (mxGetNumberOfDimensions(Tinv) > 3) ||
            ...
            mexErrMsgIdAndTxt("Images:inv_lwm:badLWMTdata", "%s",ERR_LWM_TDATA_BAD);
        }
        ...
        for (k = 0; k < mxGetNumberOfElements(Tinv); k++) {
            if (! mxIsFinite(pr_Tinv[k])) {
                mexErrMsgIdAndTxt("Images:inv_lwm:lwmTdataHasNansOrInfs", "%s",ERR_LWM_TDATA_NANINF);
            }
        }
    ...
    if ((ControlPoints != NULL) && !mxIsEmpty(ControlPoints)) {
        ...
        {
            mexErrMsgIdAndTxt("Images:inv_lwm:badControlpoints", "%s",ERR_CONTROLPOINTS_BAD);
        }
    ...
    if ((RadiiOfInfluence != NULL) && !mxIsEmpty(RadiiOfInfluence)) {
        ...
        {
            mexErrMsgIdAndTxt("Images:inv_lwm:badRadiiOfInfluence", "%s",ERR_RADIIOFINFLUENCE_BAD);
        }
        ...
        for (k = 0; k < nControlPoints; k++) {
            if (! mxIsFinite(pr_Radii[k])) {
                mexErrMsgIdAndTxt("Images:inv_lwm:radiiOfInfluenceHaveNansOrInfs",
                                  "%s",ERR_RADIIOFINFLUENCE_NANINF);
            }

            if (pr_Radii[k]<=0.0) {
                 mexErrMsgIdAndTxt("Images:inv_lwm:radiiOfInfluenceAreNegative",
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    if (nrhs < 2)
        mexErrMsgIdAndTxt("Images:inv_lwm:tooFewInputs", "%s",ERR_TOO_FEW_INPUTS);

    else if (nrhs == 2)
        TransformPoints(nlhs, plhs, nrhs, prhs);

    else
        mexErrMsgIdAndTxt("Images:inv_lwm:tooManyInputs", "%s",ERR_TOO_MANY_INPUTS);
}
