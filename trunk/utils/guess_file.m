...
% For ascii files test if they are multisegment
if bin == 0
    if isunix,      s = unix(['grep ">" ' fiche ' > ' f_path 'lix']);
    elseif ispc,   s = dos([home_dir filesep 'm_grep ">" ' fiche ' > ' f_path 'lix']);
    else            errordlg('Unknown platform.','Error');
    end
    if ~(isequal(s,1)),     multi_seg = 1;      end                 % A multisegment file
end
