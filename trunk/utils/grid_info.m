...
% elseif (handles.image_type == 2)    % "Trivial" image type (e.g. jpg, png, bmp, etc...)
%     try
%         info_img = imfinfo(ud.Iname);
%         msgbox({['File Name:    ' info_img.Filename]; []; ['File Size:    ' num2str(info_img.FileSize) '  Bytes']; []; ...
%             ['Width:  ' num2str(info_img.Width) '    Height:  ' num2str(info_img.Height)]; []; ...
%             ['Bit Depth:  ' num2str(info_img.BitDepth)]; []; ['Color Type:  ' info_img.ColorType]}, ...
%             'Image Info')
%     catch                           % Come here when MrSid, ECW or JPK2 "raw" images
%         if (~isempty(ud))
%             w{1} = ud.Driver;
%             w{2} = ['Width:  ' num2str(ud.Dim_nx) '    Height:  ' num2str(ud.Dim_ny)];
%             w{3} = ['Color Type:  ' ud.Cmap];
%             msgbox(w,'Image Info');
%         else
%             dims = size(handles.origFig);
%             w{1} = ['Width:  ' num2str(dims(2)) '    Height:  ' num2str(dims(1))];
%             w{2} = '';
%             if (numel(dims) == 3)
%                 w{3} = ['Color Type:  RGB'];
%             else
%                 w{3} = ['Color Type:  Indexed'];
%             end
%             msgbox(w,'Image Info');
%         end
%     end
% elseif (handles.image_type >= 3)    % GeoTIFF or DEM FILES
%     try      w{1} = ud.Driver;       % It may hapen
%     catch
%         dims = size(handles.origFig);
%         w{1} = 'Image Info only';       w{2} = '';
%         w{3} = ['Width:  ' num2str(dims(2)) '    Height:  ' num2str(dims(1))];
%         w{4} = ['   Xmin:  ' num2str(handles.head(1)) '    Xmax: ' num2str(handles.head(2))];
%         w{5} = ['   Ymin:  ' num2str(handles.head(3)) '    Ymax: ' num2str(handles.head(4))];
%         w{6} = '';
%         if (numel(dims) == 3)
%             w{7} = ['Color Type:  RGB'];
%         else
%             w{7} = ['Color Type:  Indexed'];
%         end
%         msgbox(w,'Image Info');
%         return
%     end
%     w{2} = ['Width:  ' num2str(ud.Dim_nx) '    Height:  ' num2str(ud.Dim_ny)];
%     w{3} = ['Projection:  ' ud.projection];
%     w{4} = ['Datum:  ' ud.datum];
%     w{5} = ['Ellipsoide:  ' ud.ellipsoid];
%     w{6} = ['Pizel Size:  (' num2str(ud.pixel_x) ',' num2str(ud.pixel_y) ')'];
%     w{7} = 'Projected corner coordinates';
%     w{8} = ['   Xmin:  ' num2str(ud.LL_prj_xmin) '    Xmax: ' num2str(ud.UR_prj_xmax)];
%     w{9} = ['   Ymin:  ' num2str(ud.LL_prj_ymin) '    Ymax: ' num2str(ud.UR_prj_ymax)];
%     if ~isempty(ud.UL_geo_xmin)
%         w{10} = 'Geographical corner coordinates';
%         w{11} = ['   Xmin:  ' num2str(ud.LL_geo_xmin) '    Xmax: ' num2str(ud.LR_geo_xmax)];
%         w{12} = ['   Ymin:  ' num2str(ud.LL_geo_ymin) '    Ymax: ' num2str(ud.UR_geo_ymax)];
%     end
%     try        w{13} = ['Zmin:  ' ud.Zmin '   Zmax: ' ud.Zmax];    end
%     w{14} = ['Color Type:  ' ud.Cmap];
%     w{15} = [];    w{16} = [];
%     [PATH,FNAME,EXT] = fileparts(ud.Iname);
%     if isempty(home_dir)                                % When this was called directly
%         curr_dir = pwd;     prefix = [curr_dir filesep];
%     else        prefix = [];
%     end
%     
%     w{16} = ['NOTE: The file "' prefix handles.path_tmp FNAME '.info" contains more detailed information about this Image'];
%     msgbox(w,'Grid Info');
% else
%     msgbox('Info missing or nothing to info about?','???')
end
