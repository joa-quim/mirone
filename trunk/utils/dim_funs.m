...
function edit_x_min(hObject, handles)
	...
	if ~isempty(val)            % when dd:mm or dd:mm:ss was given
        ...
        if (x_min < x_min_or);  set(hObject,'String',sprintf('%.8g',x_min_or));    return;     end; 
        handles.x_min = x_min;
...
function edit_x_max(hObject, handles)
	...
	if ~isempty(val)
        ...
        if (x_max > x_max_or);  set(hObject,'String',sprintf('%.8g',x_max_or));    return;     end; 
        handles.x_max = x_max;
...
function edit_y_min(hObject, handles)
	...
	if ~isempty(val)
        ...
        if (y_min < y_min_or);  set(hObject,'String',sprintf('%.8g',y_min_or));    return;     end; 
        handles.y_min = y_min;
...
function edit_y_max(hObject, handles)
	...
	if ~isempty(val)
        ...
        if (y_max > y_max_or);  set(hObject,'String',sprintf('%.8g',y_max_or));    return;     end; 
        handles.y_max = y_max;
