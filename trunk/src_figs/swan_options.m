...
% --------------------------------------------------------------------
	...
	if (numel(varargin) > 1)
		...
		elseif (strcmp(varargin{2},'Tsun2'))
			...
			set(handles.edit_SwanParams,'TooltipString','Enter a Tsun2 parameter file name')
			handles.BatGridInMemory = 1;
			set(handles.check_wantMaregs,'Value',1)
			...
			set(handles.edit_jumpInitial,'TooltipString','Jump these number of seconds from the begining of Maregraphs file')
	...
	hhs = findobj(hObject, 'UserData', 'params');
	hTab = findobj(handles.tab_group,'UserData','params'); 		% remove the "Params" tab push button from the hhs list
...
function edit_BatGrid_Callback(hObject, eventdata, handles)
	...
	if ~isempty(fname)
		push_BatGrid_Callback(handles.push_BatGrid, eventdata, handles, fname)
	end
...
function edit_SourceGrid_Callback(hObject, eventdata, handles)
	...
	push_SourceGrid_Callback(handles.push_SourceGrid, eventdata, handles, fname)

% -----------------------------------------------------------------------------------------
function push_SourceGrid_Callback(hObject, eventdata, handles)
	str1 = {'*.grd;*.GRD', 'Grid files (*.grd,*.GRD)';'*.*', 'All Files (*.*)'};
	[FileName,PathName] = put_or_get_file(handles,str1,'Select grid','get');
	if isequal(FileName,0),		return,		end
	fname = [PathName FileName];
...
function edit_SwanParams_Callback(hObject, eventdata, handles)
	fname = get(hObject,'String');
	if isempty(fname),  return,		end
	push_paramsFile_Callback(handles.push_paramsFile, [], handles, fname)
...
% -----------------------------------------------------------------------------------------
function radio_surfLevel_Callback(hObject, eventdata, handles)
...
% -----------------------------------------------------------------------------------------
function radio_maxWater_Callback(hObject, eventdata, handles)
	if (get(hObject,'Value'))
...
% -----------------------------------------------------------------------------------------
function radio_totalWater_Callback(hObject, eventdata, handles)
	if (get(hObject,'Value'))
		set(handles.radio_surfLevel,'Value',0)
		set(handles.radio_maxWater,'Value',0)
	else
...
% -----------------------------------------------------------------------------------------
function edit_jumpInitial_Callback(hObject, eventdata, handles)
...
% -----------------------------------------------------------------------------------------
function check_wantMaregs_Callback(hObject, eventdata, handles)
...
function edit_MaregraphPosFile_Callback(hObject, eventdata, handles)
	fname = get(hObject,'String');
	if isempty(fname),  return,		end
	push_MaregraphPosFile_Callback(handles.push_MaregraphPosFile, [], handles, fname)
...
function edit_MaregraphDataFile_Callback(hObject, eventdata, handles)
	if (handles.is_tsun2)
		fname = get(hObject,'String');
		if isempty(fname),  return,		end
		push_MaregraphDataFile_Callback(handles.push_MaregraphDataFile, [], handles, fname)
	end
...
% -----------------------------------------------------------------------------------------
function radio_outGrids_Callback(hObject, eventdata, handles)
	if ( ~get(hObject,'Value') )
...
function radio_anuga_Callback(hObject, eventdata, handles)
	...
	set(handles.edit_gridNameStem,'TooltipString','Output file name')
	set([handles.radio_surfLevel handles.radio_maxWater handles.radio_totalWater ...
...
% -----------------------------------------------------------------------------------------
function radio_most_Callback(hObject, eventdata, handles)
	if ( ~get(hObject,'Value') )
		set(handles.edit_gridNameStem, 'String','', 'TooltipString','')
		return
	end
	...
	set(handles.edit_gridNameStem,'TooltipString','Basename for MOST files (do not give extension)')
	set([handles.radio_surfLevel handles.radio_maxWater handles.radio_totalWater ...
			handles.check_velocity handles.check_momentum], 'Val',0)
	set([handles.radio_surfLevel handles.radio_maxWater handles.radio_totalWater ...
			handles.check_velocity handles.check_momentum], 'Enable','off')
...
% -----------------------------------------------------------------------------------------
function edit_grn_Callback(hObject, eventdata, handles)
	xx = str2double(get(hObject,'String'));
	if (isnan(xx) || xx < 0)
...
% -----------------------------------------------------------------------------------------
function edit_cumint_Callback(hObject, eventdata, handles)
	xx = str2double(get(hObject,'String'));
	if (isnan(xx) || xx < 0)
...
% -----------------------------------------------------------------------------------------
function radio_cartesian_Callback(hObject, eventdata, handles)
	if ( ~get(hObject,'Value') )
...
% -----------------------------------------------------------------------------------------
function radio_geog_Callback(hObject, eventdata, handles)
	if ( ~get(hObject,'Value') )
		set(hObject,'Value',1),		return
	end
...
'Name','Swan options',...
'NumberTitle','off',...
'Position',[520 359 682 365],...
'Resize','off',...
'HandleVisibility','callback',...
'Tag','figure1');

uicontrol('Parent',h1, 'Position',[10 333 66 23],...
'Enable','inactive',...
'String','Main',...
'ButtonDownFcn',{@swan_options_uicallback,h1,'tab_group_ButtonDownFcn'},...
'Tag','tab_group',...
'UserData','main');

uicontrol('Parent',h1, 'Position',[80 333 80 23],...
'Enable','inactive',...
'String','Parameters',...
'ButtonDownFcn',{@swan_options_uicallback,h1,'tab_group_ButtonDownFcn'},...
'Tag','tab_group',...
...
uicontrol('Parent',h1,...
'Callback',{@swan_options_uicallback,h1,'push_BatGrid_Callback'},...
...
uicontrol('Parent',h1,...
'Callback',{@swan_options_uicallback,h1,'push_SourceGrid_Callback'},...
'Position',[331 273 23 23],...
'Tag','push_SourceGrid',...
'UserData','main');

uicontrol('Parent',h1,...
'Callback',{@swan_options_uicallback,h1,'radio_outGrids_Callback'},...
'Position',[31 230 82 15],...
'String','Output grids',...
'Style','radiobutton',...
'TooltipString','Save output  as a series of Surfer format grids',...
'Tag','radio_outGrids',...
'UserData','main');

uicontrol('Parent',h1,...
'Callback',{@swan_options_uicallback,h1,'radio_anuga_Callback'},...
'Position',[141 230 88 15],...
'String','ANUGA .sww',...
'Style','radiobutton',...
'TooltipString','Save output as a single netCDF file using the ANUGA''s .sww format',...
'Tag','radio_anuga',...
'UserData','main');

uicontrol('Parent',h1,...
'Callback',{@swan_options_uicallback,h1,'radio_most_Callback'},...
'Position',[261 230 70 15],...
'String','MOST .nc',...
'Style','radiobutton',...
'TooltipString','Save output as 3 netCDF files as produced by MOST',...
'Tag','radio_most',...
'UserData','main');
...
'Style','edit',...
'TooltipString','Grids are created using this name stem',...
'Tag','edit_gridNameStem',...
'UserData','main');

uicontrol('Parent',h1,...
'Callback',{@swan_options_uicallback,h1,'radio_surfLevel_Callback'},...
'Enable','off',...
'Position',[19 169 82 15],...
...
uicontrol('Parent',h1,...
'Callback',{@swan_options_uicallback,h1,'radio_maxWater_Callback'},...
'Enable','off',...
...
uicontrol('Parent',h1,...
'Callback',{@swan_options_uicallback,h1,'radio_totalWater_Callback'},...
'Enable','off',...
'Position',[258 169 82 15],...
'String','Total water',...
'Style','radiobutton',...
...
uicontrol('Parent',h1, 'Position',[258 149 82 15],...
'Enable','off',...
'String','Momentum',...
'Style','checkbox',...
...
uicontrol('Parent',h1, 'Position',[20 134 76 15],...
'Callback',{@swan_options_uicallback,h1,'check_wantMaregs_Callback'},...
'String','Maregraphs',...
'Style','checkbox',...
...
uicontrol('Parent',h1,...
'Callback',{@swan_options_uicallback,h1,'push_MaregraphPosFile_Callback'},...
...
'Callback',{@swan_options_uicallback,h1,'push_MaregraphDataFile_Callback'},...
'Tag','push_MaregraphDataFile',...
'UserData','main');

uicontrol('Parent',h1,...
'Position',[118 51 60 15],...
'String','Nº of cycles',...
'Style','text');
...
'Style','edit',...
'TooltipString','Enter a Swan parameter file name.',...
'Tag','edit_SwanParams',...
'UserData','params');

uicontrol('Parent',h1,...
'Callback',{@swan_options_uicallback,h1,'push_paramsFile_Callback'},...
'Position',[651 103 23 23],...
'Tag','push_paramsFile',...
'UserData','params');

uicontrol('Parent',h1,...
'Position',[10 51 85 15],...
'String','Make a movie',...
'Style','checkbox',...
'Tag','check_makeMovie');
...
uicontrol('Parent',h1,...
'BackgroundColor',[1 1 1],...
'Callback',{@swan_options_uicallback,h1,'edit_jumpInitial_Callback'},...
'CData',[],...
'Position',[308 46 51 21],...
'String','0',...
'Style','edit',...
'TooltipString','Output is produced only after this modeling time (seconds) has elapsed',...
'Tag','edit_jumpInitial');

uicontrol('Parent',h1,...
'HorizontalAlignment','right',...
'Position',[250 50 55 15],...
'String','Jump initial',...
'Style','text');
...
uicontrol('Parent',h1,...
'BackgroundColor',[1 1 1],...
'Callback',{@swan_options_uicallback,h1,'edit_dt_Callback'},...
'Position',[433 265 41 21],...
'Style','edit',...
'TooltipString','Time step for the modeling run',...
'Tag','edit_dt',...
'UserData','params');

uicontrol('Parent',h1,...
'BackgroundColor',[1 1 1],...
'Callback',{@swan_options_uicallback,h1,'edit_grn_Callback'},...
'Position',[620 266 31 21],...
'String','10',...
'Style','edit',...
'TooltipString','save grids at this cycle interval (it depends on dt)',...
'Tag','edit_grn',...
'UserData','params');

uicontrol('Parent',h1,...
'Callback',{@swan_options_uicallback,h1,'radio_cartesian_Callback'},...
'Position',[380 191 82 15],...
'String','Cartesian',...
'Style','radiobutton',...
'TooltipString','Select this if your grids are in cartesian (eg UTM) coordinates',...
'Value',1,...
'Tag','radio_cartesian',...
'UserData','params');

uicontrol('Parent',h1,...
'Callback',{@swan_options_uicallback,h1,'radio_geog_Callback'},...
'Position',[501 191 82 15],...
'String','Geographic',...
'Style','radiobutton',...
'TooltipString','Select this if your grids are in geographical coordinates',...
'Tag','radio_geog',...
'UserData','params');

uicontrol('Parent',h1,...
'HorizontalAlignment','left',...
'Position',[380 261 51 30],...
...
uicontrol('Parent',h1,...
'BackgroundColor',[1 1 1],...
'Callback',{@swan_options_uicallback,h1,'edit_cumint_Callback'},...
'Position',[620 230 31 21],...
'String','1',...
'Style','edit',...
'TooltipString','cumint * dt = time at which the virtual tide gauges are writen',...
'Tag','edit_cumint',...
'UserData','params');

uicontrol('Parent',h1,...
'HorizontalAlignment','left',...
'Position',[500 222 112 31],...
...
uicontrol('Parent',h1,...
'Callback',{@swan_options_uicallback,h1,'push_OK_Callback'},...
'FontWeight','bold',...
'Position',[214 8 66 23],...
'String','OK',...
'Tag','push_OK');

uicontrol('Parent',h1,...
'Callback',{@swan_options_uicallback,h1,'push_Cancel_Callback'},...
'FontWeight','bold',...
'Position',[294 8 66 23],...
'String','Cancel',...
'Tag','push_Cancel');
