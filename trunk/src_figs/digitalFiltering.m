...
digitalFiltering_LayoutFcn(hObject,handles);
handles = guihandles(hObject);
...
str1 = {'*.bmp', 'Windows Bitmap (*.bmp)'; ...
    ...
    '*.tif', 'Tagged Image File (*.tif)'; ...
    '*.xwd', 'X Windows Dump (*.xwd)'};
[FileName,PathName] = uiputfile(str1,'Select image format');
if isequal(FileName,0);     return;     end
...
'FontSize',9,...
'Position',[10 71 251 335],...
'Style','listbox',...
'Value',1,...
'Tag','listbox1');

uicontrol('Parent',h1,'FontSize',10,'Position',[31 408 199 16],...
'String','Filters','Style','text','Tag','text4');

uicontrol('Parent',h1,...
'Callback',{@digitalFiltering_uicallback,h1,'pushbutton_cancel_Callback'},...
'Position',[491 9 50 23],...
'String','Cancel',...
'TooltipString','Stop and undo any previous changes',...
...
'String','...',...
'TooltipString','Browse for an image',...
'Tag','pushbutton_imageIn');

uicontrol('Parent',h1,'Position',[644 405 67 15],...
'String','Original Image','Style','text','Tag','text11');

uicontrol('Parent',h1,'Position',[646 196 70 15],...
'String','Filtered Image','Style','text','Tag','text12');

uicontrol('Parent',h1,...
'Callback',{@digitalFiltering_uicallback,h1,'pushbutton_apply_Callback'},...
'Position',[270 10 50 23],...
'String','Apply',...
'TooltipString','Filter image (or grid)',...
'Tag','pushbutton_apply');

uicontrol('Parent',h1,...
'Callback',{@digitalFiltering_uicallback,h1,'pushbutton_apply_and_return_Callback'},...
'Position',[350 10 77 23],...
'String','Apply n return',...
'TooltipString','Stop and return filtered image and grid (if required)',...
'Tag','pushbutton_apply_and_return');

uicontrol('Parent',h1,...
'Callback',{@digitalFiltering_uicallback,h1,'pushbutton_saveFiltImg_Callback'},...
'Position',[431 287 110 23],...
...
'Position',[310 287 110 23],...
'String','Load external filter',...
'TooltipString','Import an external (MxN) filter',...
