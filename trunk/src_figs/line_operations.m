...
% --------------------------------------------------------------------
	...
	handles.known_ops = {'buffer'; 'bspline'; 'cspline'; 'polyunion'; 'polyintersect'; ...
	...
	handles.ttips{3} = sprintf(['Smooth line with a B-form spline\n' ...
								'It will open a help control window to\n' ...
								'help with selection of nice parameters']);
	handles.ttips{4} = sprintf(['Smooth line with a cardinal spline\n' ...
	...
	handles.ttips{5} = 'Performs the boolean operation of Union to the selected polygons.';
	handles.ttips{6} = 'Performs the boolean operation of Intersection to the selected polygons.';
	handles.ttips{7} = 'Performs the boolean operation of exclusive OR to the selected polygons.';
	handles.ttips{8} = 'Performs the boolean operation of subtraction to the selected polygons.';
	handles.ttips{9} = 'Convert line objects into patch. Patches, for example, accept fill color.';
	handles.ttips{10} = sprintf(['Dray a polyline with vertices defined by coords [x1 xn; y1 yn].\n' ...
								'Note: you must use the brackets and semi-comma notation as above.\n' ...
								'Example vector: [1 1.5 3.1; 2 4 8.4]']);
	handles.ttips{11} = sprintf(['Send the selected object handles to the Matlab workspace.\n' ...
								'Use this if you want to gain access to all handle properties.']);

	if (IamCompiled),	handles.known_ops(10) = [];	handles.ttips(11);	end		% regretably
...
'String',{'Possible commands'; 'buffer DIST'; 'bspline'; 'cspline N RES'; 'polyunion'; 'polyintersect'; 'polyxor'; 'polyminus'; 'line2patch'; 'pline [x1 ..xn; y1 .. yn]'; 'hand2Workspace' },...
'Style','popupmenu',...
